---
title: Basic charts
description:
  Basic charts offer a way to visualize data sets in an intuitive, easy to
  understand way
---

<PageDescription>

Basic charts offer a way to visualize data sets in an intuitive, easy to
understand way. Every chart should tell a story and should reflect the content
on the page where it is found.

</PageDescription>

import demoGroups from '../../../data/data-visualization';

import { AnchorLinks, AnchorLink } from 'gatsby-theme-carbon';
import AllDemos from '../../../components/data-visualization/AllDemos.js';

<AnchorLinks>
  {
    demoGroups.map(storybookDemoGroup => (
      <AnchorLink>{storybookDemoGroup.title}</AnchorLink>
    ))
  }

  <AnchorLink>Design-only</AnchorLink>
</AnchorLinks>

## Resources

<Row className="resource-card-group">
  <Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      subTitle="Carbon Charts demo site"
      href="https://carbon-design-system.github.io/carbon-charts/">
      <MdxIcon name="codesandbox" />
    </ResourceCard>
  </Column>
</Row>

<AllDemos />

## Design-only

<InlineNotification>

Note: These charts are currently a work-in-progress. To see our roadmap, request
missing guidance, or contribute content, please go to the carbon-charts
[GitHub repository](https://github.com/carbon-design-system/carbon-charts).

</InlineNotification>

### Meter

<InlineNotification>

**Note:** Meter charts are not yet available in Carbon Charts. These
explorations are subject to change.

</InlineNotification>

Meter charts are useful for showing values between a small number of variables
either by using multiple markers on the same meter, or by using multiples of the
chart.

<Row>
<Column colMd={4} colLg={4}>

![Meters](images/charts-basic-meter.png)

<Caption>
Meter charts can represent data with current value, minimum and maximum, and peaks or averages.
</Caption>
</Column>
</Row>
