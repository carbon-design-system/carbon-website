...
formats: Simple (single step) and Multistep options, including Modal, Narrow
Tearsheet, Wide Tearsheet, and Full-page formats.
...
Narrow or Wide Tearsheet variations for single or multi-step options,
respectively.
...
1. **Modal size & overlay:**
   [Modal size "small"](https://www.carbondesignsystem.com/components/modal/style#small).
...
- The user will be taken to a different page after creation.

### When not to use

- The create can be performed inline.
...
1. **Panel size & overlay**: Should follow the
   [Modal size "small"](https://www.carbondesignsystem.com/components/modal/style#small).
   See [Narrow Tearsheet documentation](none yet). Screen overlay that obscures
   the on-page content.
2. **Create title**: Title line for the create flow with an optional subtitle.
   The title will usually be "Create `asset type`".
3. **Body**: Contains the information and/or controls needed to populate the
   task. It can include message text and components.
...
- There are more fields than can fit into the modal without scrolling.
- The steps can be broken down into sections or chapters.

### When not to use

- The flow has high complexity and the user would benefit from having a visible
  progress indicator.
...
1. **Tearsheet & overlay**: [Tearsheet component](none yet) with screen overlay
   that obscures the on-page content.
2. **Create title**: Title line for the create flow with an optional subtitle.
   The title will usually be "Create `asset type`".
3. **Progress indicator**: Vertical progress indicator lets the user know what
   step they are on. The default left panel width is 256px. An option of 310px
...
- There are many form fields to be filled out.

### When not to use

- The create flow has low complexity.
...
- In Cloud and Cognitive, the create flow is the process the user must complete
...
- The product or service is usable before the current creation flow is
  initiated.
