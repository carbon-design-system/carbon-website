...
![Non-exhaustive examples of buttons that launch an Export modal](images/ButtonInteractions.png)

<Caption>Non-exhaustive examples of buttons that launch an Export modal</Caption>
</Column>
</Row>
...
![Modal focus order](images/ModalInteractions.png)
...
#### If a sensible default value is selected or input for the user (the user is not required to complete any fields in order to export):
...
3. On `tab`, apply focus to the first field that accepts user input. Follow
   accessibiltiy guidance for the interaction type of this field (i.e.
...
![Focus on the close icon when using passive modals](images/FailureInteractions.png)
