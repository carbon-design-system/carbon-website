...
WAI-ARIA roles and states are used to ensure the component is accessible to
people using assistive technologies. The accordion component is assigned the
ARIA `role="presentation"` because it is used to layout web content. When an
accordion panel is collapsed the content is hidden so the ARIA state,
`aria-expanded="false"` advises users of assistive technologies that the
...
have a aria-control property set that points to the unique id of the panel it
