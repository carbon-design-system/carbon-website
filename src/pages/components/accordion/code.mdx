...
import usage from './snippets/usage';
import skeleton from './snippets/skeleton';

<PageDescription>

Learn how to build an accordion in React.
...
`AccordionItem` components. The `Accordion` components acceps a list of
...
<ComponentDemo links={{}}>{usage}</ComponentDemo>
...
Additional props passed into `AccordionItem`, like `onClick`, will be passed
...
| --------- | ------------------------ | -------- | ------- |
| align     | `'start'` &#124; `'end'` | `false`  | `'end'` |
| className | `string`                 | `false`  |         |
| count     | `boolean`                | `false`  | `4`     |
...
<h3 id="interactive-content-violation">
...
If you render an interactive element inside of this header, then it becomes
nested inside of the `<button>`. Common examples of this are buttons or links.
Rendering interactive content inside of the accordion heading is not reachable
