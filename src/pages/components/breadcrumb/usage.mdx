...
All the pages in the breadcrumb component should be interactive (except the current page) and link to
...
Users can navigate between breadcrumb links by pressing `Tab` and `Shift-Tab`. Users can trigger a breadcrumb link by pressing `Enter` while the link has
