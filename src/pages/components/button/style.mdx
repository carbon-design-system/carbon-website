...
<Title>Danger ghost button button interactive state color</Title>
...
borders. For a button with a glyph, the space between the button label and the
glyph must be greater than or equal to 16 pixels / 1 rem. This is to accommodate
for instances where two or more buttons with glyphs appear together.
...
<Caption>Structure measurements for a contained button | px / rem</Caption>
