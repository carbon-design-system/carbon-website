...
contained button the label is always left-aligned, not centered. By default
...
The label is right-aligned, the icon is left-aligned.
...
<Caption>Example of a primary button with and without RTL.</Caption>
...
<Caption>
  Primary button taking default focus when there’s no other actionable component
  in a dialog.
</Caption>

### States

The default and interactive states of menu buttons should follow the
button [Style tab](/components/button/style#color.mdx) guidelines.
...
<Caption>
  Example of icon only copy button of code snippet conveying validation through
  a tooltip.
</Caption>
...
<Caption>Example of inline loading in context of a button.</Caption>

## Primary button

Primary buttons are used for the principal call to action on the page. Primary
buttons should only appear once per screen (the only exceptions are temporary
...
<Caption>Primary button in context of a product UI.</Caption>
...
acceptable. This is the only case where having two primary actions on a page is
advised.
...
<Caption>
  Primary intent of this page is to read content, with actions like filtering
...
<Caption>Secondary button in context of a product UI.</Caption>
...
<Caption>Tertiary button in context of a product UI.</Caption>
...
placement at the top. Therefore, it is advised to use a tertiary button for page
headers.
...
<Caption>Ghost button in context of a product UI.</Caption>

#### Aligning ghost buttons

Ghost buttons work well when aligned to a corner of a container. The general
rule for vertically aligning a ghost button with other content is to ensure the
label of the button vertically aligns with text elsewhere on the page.
...
![Do align the ghost button label with the rest of the content.](images/button_usage_ghost_2.png)
  ...
  <DoDont type="dont" caption="Do not align the edge of the ghost button with the rest of the content, even if it looks hanging.">
...
In side panels, ghost buttons can be helpful for small supplementary actions. In
...
side panels of 480px (medium) and below.

<DoDontRow>
  <DoDont type="do" caption="Do extend the ghost button to the full width of side panel.">

![Do extend the ghost button to the full width of side panel.](images/button_usage_ghost_4.png)

  </DoDont>
  <DoDont type="dont" caption="Do extend the ghost button to the full width of side panel.">
...
See
the [cards component](https://pages.github.ibm.com/cdai-design/pal/components/card/overview)
for more details.
...
to give to the danger action. Destructive actions that are considered a required
or primary step in a workflow should use the primary danger button style.
However, if a destructive action is just one of several actions a user could
choose from, then a lower emphasis style like the tertiary danger button or the
...
<Caption>Danger button in context of a product UI.</Caption>
...
us know [here](https://github.ibm.com/CDAI-design/pal).

Icons that are not in this list still can be used in buttons, as long as the
icon is clearly tied to the intended action. To determine the expected use of an
icon, check its name in the
[Carbon icon library](https://www.figma.com/design/J5c0d85dSJn9JnBhSYYLmD/IBM%C2%AE-UI-Icon-Library?m=auto).
...
<DoDontRow>
  <DoDont type="do" caption="Do use a defined icon for a universal action.">

![Do use a defined icon for a universal action.](images/button_usage_button_with_icon_5.png)

  </DoDont>
  <DoDont type="dont" caption="Do not add an icon for an action that would not be associated with it.">
...
are often found in the UI left navigation area, in side panels, on cards, and in
modals. The target destination of the launch action should be made clear to the
user through the button or link label and the surrounding context. For more
guidance, see this section of our content guide.
...
<DoDontRow>
  <DoDont type="do" caption="Do follow a consistent approach across button groups for introducing icons.">
...
action with a strong visual attribute – such as a pin icon to represent a
pinning action. There is insufficient space and multiple actions, therefore a
...
[Style tab](/components/button/style#ghost-button-color.mdx).
...
the [danger button guidance](#danger-button),  its best to use a danger button
...
![Do use a visual label in a danger button for a destructive action.](images/button_usage_icon_only_button_3.png)
  ...
  <DoDont type="dont" caption="Do not use danger button in an icon only form for a destructive action.">
