...
Users can trigger a state change by pressing the `Enter` key while the checkbox input has focus. For additional keyboard interactions, see the [accessibility tab](https://www.carbondesignsystem.com/components/checkbox/accessibility).
...
JAWS: Users can trigger a state change by pressing `Enter` key while the checkbox input has screen reader focus.

NVDA: Users can trigger a state change by pressing `Enter` key while the checkbox input has screen reader focus.
