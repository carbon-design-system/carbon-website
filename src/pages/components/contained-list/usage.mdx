...
The on page list comes in three height sizes — extra large (64px), large (48px),
and small (32px). The large height size is the default size.
...
but by default each list item row height follows the extra large, large or small
height size for one line of content.
...
There are two different lengths for row dividers — extended and indented. By
default, use extended row dividers. Typically indented row dividers are used
when placed close to other components that also contain row dividers so they do
not collide with each other and create visual tension.

<Row>
<Column colLg={8}>

![Contained list row divider length options](images/contained-list-usage-11.png)
...
filter can collapse and expand or stay persistent on the page, depending on the
use case.
