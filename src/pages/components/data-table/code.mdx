...
<AnchorLink>Sorting</AnchorLink>
<AnchorLink>Filtering</AnchorLink>
<AnchorLink>Expansion</AnchorLink>
<AnchorLink>Batch actions</AnchorLink>
...
| Prop        | Type                                           | Required | Default    | Description                                                   |
| ----------- | ---------------------------------------------- | -------- | ---------- | ------------------------------------------------------------- |
| `aria-live` | `'polite'` &#124; `'assertive'` &#124; `'off'` | –        | `'polite'` | Set the notification behavior of the table for screen readers |
| `children`  | `node`                                         | –        | –          | –                                                             |
| `className` | `string`                                       | –        | –          | –                                                             |
...
| `children`    | `node`   | –        | –       | –                                   |
| `className`   | `string` | –        | –       | –                                   |
| `description` | `node`   | –        | –       | Provide a description for the table |
...
| Prop         | Type      | Required | Default | Description                                                                                   |
| ------------ | --------- | -------- | ------- | --------------------------------------------------------------------------------------------- |
| `ariaLabel`  | `string`  | `true`   | –       | Provide a string that is read by a voice reader when the expand trigger is focused            |
| `children`   | `node`    | –        | –       | –                                                                                             |
| `className`  | `string`  | –        | –       | –                                                                                             |
| `isExpanded` | `boolean` | `true`   | `false` | Specify whether this row is expanded or not. This helps to coordinate with `TableExpandedRow` |
| `onExpand`   | `func`    | `true`   | –       | Provide a function that is called when a user interacts to expand the current row             |
...
| `children`  | `node`   | –        | –       | –                                                                    |
| `className` | `string` | –        | –       | –                                                                    |
| `colSpan`   | `number` | –        | –       | Provide a `colSpan` value that is applied to the expanded row's cell |
...
| `colSpan`         | `number`                                | –        | –       | Specify `colSpan` as a non-negative integer value to indicate how many columns the TableHeader cell extends in a table                                       |
| `isSortable`      | `bool`                                  | –        | `false` | Specify whether this header is one through which a user can sort the table                                                                                   |
| `isSortHeader`    | `bool`                                  | –        | –       | Specify whether this header is the header by which a table is sorted                                                                                         |
| `scope`           | `string`                                | `true`   | `'col'` | –                                                                                                                                                            |
| `sortDirection`   | `'DESC'` &#124; `'ASC'` &#124; `'NONE'` | –        | –       | Specify which direction we are currently sorting by                                                                                                          |
| `translateWithId` | `function`                              | –        | –       | Supply a method to translate internal strings with your i18n tool of choice. Translation keys are avabile on the `translationKeys` field for this component. |

#### TableHeader translateWithId

You can localize what is announced to a user by a screen reader with the
`translateWithId` prop. This function will take in the current key to translate,
along with state, in order to generate the appropriate label.

Below is a list of the translation ids for `TableHeader`:

| id                                     | Description                                     |
| -------------------------------------- | ----------------------------------------------- |
| `carbon.table.header.icon.description` | Specify a description used in a sortable header |

You can use the translation ids for `TableHeader` along with the provided state
information in your custom `translateWithId` function.
...
| Prop            | Type       | Required | Default                          | Description                                                                      |
| --------------- | ---------- | -------- | -------------------------------- | -------------------------------------------------------------------------------- |
| `ariaLabel`     | `string`   | `true`   | `'Select all rows in the table'` | Provide an aria label for the underlying input control                           |
| `checked`       | `boolean`  | `true`   | –                                | Specify whether all items are selected, or not                                   |
| `className`     | `string`   | –        | –                                | Provide a custom class name for the cell that wraps the underlying input control |
| `id`            | `string`   | `true`   | –                                | Provide an `id` for the underlying input control                                 |
| `indeterminate` | `boolean`  | –        | –                                | Specify whether the selection only has a subset of all items                     |
| `name`          | `string`   | `true`   | –                                | Provide a `name` for the underlying input control                                |
| `onSelect`      | `function` | `true`   | –                                | Provide a handler to listen to when a user clicks on the input                   |

### TableSelectRow props

| Prop        | Type       | Required | Default | Description                                                             |
| ----------- | ---------- | -------- | ------- | ----------------------------------------------------------------------- |
| `ariaLabel` | `string`   | `true`   | –       | Provide an aria label for the underlying input control                  |
| `checked`   | `boolean`  | `true`   | –       | Specify whether the row is selected                                     |
| `className` | `string`   | –        | –       | –                                                                       |
| `id`        | `string`   | `true`   | –       | Provide an `id` for the underlying input control                        |
| `name`      | `string`   | `true`   | –       | Provide a `name` for the underlying input control                       |
| `onSelect`  | `function` | `true`   | –       | Provide a handler to listen to when a user clicks on the input          |
| `radio`     | `boolean`  | –        | –       | Specify whether the control should be a radio button or inline checkbox |

### TableToolbar props

| Prop         | Type                        | Required | Default                | Description |
| ------------ | --------------------------- | -------- | ---------------------- | ----------- |
| `aria-label` | `string`                    | `true`   | `'data table toolbar'` | –           |
| `children`   | `node`                      | –        | –                      | –           |
| `size`       | `'small'` &#124; `'normal'` | –        | –                      | –           |

### TableToolbarAction props

This component is a wrapper for `OverflowMenuItem`.
...
This component is a wrapper for `OverflowMenu`.
...
| `children`               | `node`                   | –        | –                 | –                                                                                                                 |
| `className`              | `string`                 | –        | –                 | –                                                                                                                 |
| `defaultValue`           | `string`                 | –        | –                 | Provide the optional default value for the Search component                                                       |
| `id`                     | `string`                 | –        | –                 | Provide an optional id for the search container                                                                   |
| `labelText`              | `string`                 | –        | –                 | Provide the optional label text for the Search component icon                                                     |
| `onChange`               | `function`               | –        | –                 | Provide an optional hook that is called each time the input is updated                                            |
| `persistent`             | `boolean`                | –        | `false`           | Specify whether the search should be allowed to expand                                                            |
| `placeHolderText`        | `string`                 | –        | –                 | Provide the optional placeholder text for the Search component                                                    |
| `searchContainerClasses` | `string`                 | –        | –                 | Provide an optional className for the overal container of the Search                                              |
| `tabIndex`               | `string` &#124; `number` | –        | `'0'`             | Optional prop to specify the tabIndex of the `<Search>` (in expanded state) or the container (in collapsed state) |
| `translateWithId`        | `function`               | `true`   | `translateWithId` | Provide custom text for the component for each translation id                                                     |

#### TableToolbarSearch translateWithId

| id                                        | Description                                       |
| ----------------------------------------- | ------------------------------------------------- |
| `carbon.table.toolbar.search.label`       | Specify the label for the search field            |
| `carbon.table.toolbar.search.placeholder` | Specify the placeholder text for the search field |
