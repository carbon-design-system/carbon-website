...
build a table in React, in addition to a `DataTable` that helps you add sorting, row
expansion, selection, filtering, and more to your table.

To build a table, you can use the following set of
components. Each one matches one-to-one with the structure of a table in HTML.
...
These components provide you with a way to build a table that looks and feels like the
default table for the Carbon Design System.
...
that you then use to add functionality to your table.

As an example, we will use the `getHeaderProps` helper to create a
sortable table.
...
There are other functions available that are also examples of prop getters which
help you add additional behavior to your tables.

For a full list of what is available, check out the [DataTable
