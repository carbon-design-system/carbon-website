...
The search field can appear always open on a data table. The field is
placed on the left side of the table under the table title and stretches until
it meets the actions on the right.

<Row>
<Column colLg={8}>

![data table search](images/datatable-search-2-a.png)
...
Instead of a multiselect checkbox function, the data table supports a
single selection radio button control. Only one row can be selected at a time
when using the radio selection. The radio button is placed in the first column
on the left side.
...
When the overflow menu contains fewer than three options, keep the actions inline
