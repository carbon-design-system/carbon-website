...
Pickers are used to display past, present, or future dates or times. The kind of date (exact, approximate, memorable) you are requesting from the user will determine which picker is best to use. All picker's format can be customized depending on location or need.

### When to use

- Asking the user for a time or date
- Scheduling tasks
...
| Type              | Purpose                                                                                                                                                                                                                               |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Simple date input | Use if the date can easily be remembered by the user (like a date of birth) and they don’t need a calendar to anticipate the dates. It consists only of input fields.                                                                 |
...
<Caption>
  Anatomy of a simple date input (top) and single date calendar picker (bottom).
</Caption>
...
3. Icon
4. Calendar
5. Month and year controls
6. Previous and next month controls
7. Dates
...
1. Label
2. Hour and minute text field
3. AM/PM select
4. Timezone select

### Alignment

By default, the pickers have fixed widths. If you are placing the picker inline with other inputs, such as in form, then the widths can be adjusted to match the other inputs. The picker can either increase or decrease in width as needed. If the size is adjusted know that pickers have minimum widths, the date content should never horizontal scroll or overflow.
...
- When the using calendar picker the date format will be automated if the user selects from the calendar menu.
...
- If using the 12-hour it must be accommodated with an AM/PM selection.
...
Localization, sometimes referred to as internationalization, refers to software adapting to different languages, regional peculiarities and technical requirements of a target locale without additional code changes. This means that if the location is known then formatting of a date or time can automatically change to the acceptable local format. You should always try and design for localization.
...
The simple date input can include _month/year_ or _month/day/year._ The formatting may be localized and rearranged in sequence of appearance.

<Row>
<Column colLg={8}>

![Simple date input](images/simple-date-input.png)
...
Simple date inputs are best for when asking the user for an approximate date instead of an exact date, especially in regards to past dates. For example, when was asking a user when a purchase was made they will most likely easily recall the month and year (ie November 2019) versus the specific day (ie November 22, 2019).

### States

The simple date input is a [text input](/components/text-input/usage) and has the same interactive state and behaviors. See the style tab for more details.
...
Use a calendar picker when the user needs to know a date's relation to other days such as the day of the week it falls on or it's proximity to today. They are optimal for scheduling tasks.
...
The date range picker functions much like the single date picker but instead of choosing just one date the user can choose a `start` and `end` date. Both dates have the option to manually enter the date in a text field or select the date in the calendar. Each point requires a day, month, and year to be selected.
...
The calendar can be open in two ways:

- Clicking the `calendar` icon on the far right of the field will open the calendar menu.
- When the text field receives focus the calendar menu will also appear and remain open until a date is selected or focus is removed from the picker.
...
By default the current date and year will appear in the calendar. To navigate to another year the user can do one of the following:

- Manually typing in the year in the date text field.
- Clicking on the up and down arrows that appear when you focus or hover on the year input in the calendar.
...
- Selecting a single date or the end date in a range (will automatically close it)
- Clicking anywhere outside of calendar menu
...
- Manually type the date for the start and end dates in the text field.
- Once the calendar is open the first date you click will be the start date and second date you click will be the end date.
...
Use min and max dates to help prevent user error. If a user cannot select dates in the past when scheduling the set a min date to today.
...
Time pickers provide the user with a text field in which they can input hour and minutes. Additionally, they can be accompanied by an AM/PM and a time zone control styled as selects.

The time field format should include the hour and minutes, ie 11:30. It may be localized accommodate the 12hour or 24 hour format.

#### Use for scheduling

Use the time picker when a specific time is need to be scheduled, such as planning a meeting time.

<Row>
<Column colLg={8}>

![Time picker](images/time-picker-type.png)
...
Another common use-case not accounted for in the above pickers is setting a time or date relative to today or now, such as with a reminder. For example, setting a install to begin now, in one hour, or tonight. This type of picker can made by using the standard [select](link) component.

#### Using individual inputs

Another acceptable form of a time or date experience would be using individual inputs for each factor. Using an individual field for each factor of a date or time can help reduce formatting errors. This is especially beneficial when localization cannot be used to reformat dates.

Instead of having one input for an exact date (month/day/year), you could have three separate inputs and a clear label for each.
