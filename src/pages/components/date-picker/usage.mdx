...
- The date format can be displayed differently depending on your location. For
  example, some countries use month/day/year while others use year/month/day.
    The date picker utilizes [flatpickr](https://flatpickr.js.org/), which accepts `dateFormat` and `locale` props
  that allow you to change how the date is displayed and lets you change the
  language of the calendar. Localizing static dates in the same UI is also
  recommended if you choose to localize your date entries.
