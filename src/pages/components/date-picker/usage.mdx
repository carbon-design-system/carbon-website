...
- The date format can be displayed differently depending on your location. For
  example, some countries use month/day/year while others use year/month/day.
  The date picker uses `flatpicker` under the hood, which accepts a date format
  prop that allows you to change how the date is displayed and accepts a locale
  prop that enables you to change the language of the calendar. If you choose to
  localize your date entries, localizing any static dates in the same UI is
  recommended.
