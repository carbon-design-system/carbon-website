...
the selected option. An option with focus is selected with `Space` or `Enter`,
which also closes the dropdown. Pressing `Esc` or `Tab` closes a dropdown
...
`Enter` will select a highlighted option and collapse the combo box. (If nothing
is highlighted in the list, pressing `Enter` will just toggle the combo box open
