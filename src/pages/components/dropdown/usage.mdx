...
- It is best practice not to use a dropdown if there are two or fewer options to choose from. In this case, use a [radio button](https://www.carbondesignsystem.com/components/radio-button/code) group instead.
...
- Consider using a [select dropdown](https://www.carbondesignsystem.com/components/select/usage) if most of your experience is form based or frequently used on mobile platforms. The native select works more easily with a native form when submitting data and is also easier to use on a mobile platform.
