...
blind users and others who operate via the keyboard.
[Learn more about accessibility considerations](https://carbondesignsystem.com/components/dropdown/accessibility/?_ga=2.84465055.789214944.1712051588-758524008.1712051588&_gl=1*16jmo7x*_ga*NzU4NTI0MDA4LjE3MTIwNTE1ODg.*_ga_FYECCCS21D*MTcxMjEzODc0OC4zLjEuMTcxMjE0MTQ5OC4wLjAuMA..#what-carbon-provides)
...
The parent checkbox associated to the parent option makes it possible for user
to select all options from the list. Although the parent checkbox element is
used only for multiselect dropdowns, it is not mandated to be present in all
multiselect dropdowns.
...
- The background fill for the all button does not change when selected and will
  continue to have the same background as the active state

The parent checkbox has a third indeterminate state and appears when some and
not all options have been selected.

- Tapping on the indeterminate state clears all options.
- Tapping on the unselected checkbox selects all options.
- Tapping on the selected checkbox clears all options.
