...
| [Combo box](#combo-box)     | Allows a user to make a selection from a list of suggested, likely, or desired values. |
...
4. **Option:** A list of multiple options to select from within a menu.
5. **Menu:** Displayed as an open state and contains a list of options to select
   from.
...
- A combo box is a combination of a standard list box or a dropdown list that
  lets you type inside the input field to find an option that best represents
  your inputted value.
