---
title: Dropdown
description: Dropdowns present a list of options from which a user can select one option, or several. The selected options may represent a value in a form, or can be used as an action to filter or sort existing content on a page.
tabs: ["Usage", "Style", "Code", "Accessibility"]
---

<PageDescription>

Dropdowns present a list of options from which a user can select one option, or several. The selected options may represent a value in a form, or can be used as an action to filter or sort existing content on a page.
...
| [Combobox](#combobox)       | Allows the the user to make a selection from a predefined list of options either by typing a value directly or by selecting a value from the list. |
...
Dropdowns are composed of four distinct sections—assistive text like labels or help text, a field, a menu, and options contained within the menu. Labels and help text are optional but can guide the user to make an informed decision when making a selection.
...
1. **Labels and help text (optional):** Assistive text to help the user make a selection.
...
![Dropdown assistive labels, help text or none.](images/dropdown-usage-5.png)
...
All fields in a form are assumed required, and optional fields are tagged. For more information, see the [form component](https://www.carbondesignsystem.com/components/form/usage) usage guidance.
...
- Users trigger a `dropdown` menu to open or close by clicking the chevron icon or clicking anywhere within the field. _Note: You can only open or close a combobox by clicking the chevron icon._
...
- Users can open the dropdown menu by pressing the `Spacebar`, `Enter`, `Down arrow` or `Up Arrow`.
- Users can close the dropdown menu by pressing the `Escape`, `Spacebar`, or `Enter`

#### Combobox and filtering:

- Type a character: focus stays on the field while an option in the menu is highlighted that best matches the typed character.
- Type multiple characters in rapid succession: focus stays on the field while an option in the menu is highlighted that best matches the string of characters typed.

#### Multiple selection:

- The multiselect field receives focus if none of the options are selected in the menu.
- Once an option in the dropdown menu receives focus, users can press `Enter` to select or deselect the option.
- Users can open and close a dropdown menu by pressing the `Spacebar`.

#### Screen readers

Labels must be announced to the screen reader on focus. Ensure the help text that appears under an input is read when an assistive technology user stops at an input using ARIA.
...
When placing a dropdown inline with other content use the inline modifier. If adding a visual label to an inline dropdown it should appear inline to the left of the dropdown. If there is no visual label present you must supply an appropriate accessibility label to the inline dropdown. _Note: Inline is only a modifier for dropdown and does not have filtering functionality._
...
-[Checkbox](https://www.carbondesignsystem.com/components/checkbox/code) -[Form](https://www.carbondesignsystem.com/components/form/code) -[Radio button](https://www.carbondesignsystem.com/components/radio-button/code) -[Select](https://www.carbondesignsystem.com/components/select/code)
