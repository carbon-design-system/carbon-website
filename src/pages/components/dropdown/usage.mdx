...
_Dropdowns_ present a list of options that can be used to filter or sort
existing content.

They can also be used as menus. Their use as menus can be seen in
[tabs](/components/tabs/code). At a smaller screen size, the tabs collapse into
a dropdown.

### Dropdown versus Select

While the dropdown and select components look similar, they have different
functions.
...
The dropdown component is styled to match the design system, while the select
component's appearance will be determined by the browser being used.
