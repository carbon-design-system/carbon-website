...
By default, the drag and drop file uploader allows users to upload multiple
files at a time. If the requirement only allows a single file to be uploaded,
drag the single file into the drop zone area to upload it. Once uploaded, the
drop zone area will be removed to show that you have successfully uploaded a
single file. Click the close icon in the uploaded file to remove it.
