...
<AnchorLink>Additional form guidance</AnchorLink>
...
>
  ...
  <Stack gap={7}>
    <TextInput
      ...
      placeholder="Optional laceholder text"
    />
  <TextArea
    cols={50}
    helperText="Optional helper text here; if message is more than one line text should wrap (~100 character count maximum)"
...
Fluid inputs are an exception to the no critial information in tooltips rule.
Due to their architectural nature, fluid inputs do not provide the allotted
space under the field needed for helper text. All assistive text wither
...
Carbon provides a variety of data input components that enable a user to make a
...
| [Form layout](/patterns/forms-pattern/#designing-a-form)                          | Additional guidance around layout, spacing, columns in a form.                                                  |
