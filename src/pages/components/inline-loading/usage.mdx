...
Common actions that could use inline loading include any create, update, or delete actions that may have a lot of data to process. It can be used in a table, after a primary or secondary button click, or even in a modal.

### When to use

- Use an inline loading component for any action that cannot be preformed instantly and will only require a short time to process.
...
When inline loading temporally replaces content it should appear in the same spot as the content it replaced and maintain the same alignment.
...
This is the text that appears while the action is happening. The label should tell the user what action is being preformed. For example, if it appears while a form is being save then it should say "Saving ..."
...
There are four states to inline loading: inactive, active, success, error.
...
The success loading state indicates that the action completed successfully. The success state is active for 1.5 seconds before calling a optional function `onSuccess`. Use the `onSuccess` function to refetch new data, close a modal, or reset a form. If no function is specified the success state will last indefinitely.
...
A loading spinner can be used by itself without a label. It is most commonly used this way inside of a field input.
