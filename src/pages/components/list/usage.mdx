...
- Avoid using a list for complex, structured content, or when advanced features
  like sorting, filtering, or selection are required—consider using
...
clarity. However, when dealing with a large number of list items, multiple
columns can be used to optimize space.
...
left or right to maintain a clear structure without misalignment.
...
Markers should be top-aligned with the list item for clarity and consistency.

Unordered lists:

- Level 1 items are marked with en dashes ("-").
...
Text in a list should wrap below the counter/marker, keeping it top-aligned with
the list item to ensure readability. Truncation should be avoided and text
...
list item begins with a en dash ("-"), making it ideal for non-sequential
content.
...
item begins with a number ("1, 2, 3..."), indicating its position, making it
suitable for sequential content.
