...
Where no other content changes happen as the loading icon disappears, the change can also be surfaced to users through a non-displayed status message, such as "finished" or "loading complete," that can be conveyed to users through assistive technology. 
...
- See the Equal Access Toolkit information on [status messages](https://www.ibm.com/able/toolkit/develop/dynamic-updates/#role-status).