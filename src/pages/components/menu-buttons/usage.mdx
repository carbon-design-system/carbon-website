...
Menu buttons come in three sizes: **small**, **medium**, and **large**. Menu
items have the same size as menu buttons with an additional **extra small**
size. See more about size specifications on the
[Style](/components/menu-buttons/style/) tab. When using a menu button trigger,
the height of the menu item (singular) reflects the size of the button being
used.
