...
The tab order should proceed left and down from whichever item has focus then wrap back to the Close button (X).
...
- Carbon assigns the container a role of "dialog" and sets `aria-modal` to `"true"`.
- The dialog is labelled via `aria-label`, using the same string used for the modal’s title; `aria-labelledby` could also be used to assign the title’s string.
- See the modal pattern in the [ARIA authoring practices](https://w3c.github.io/aria-practices/#dialog_modal) for more considerations.
