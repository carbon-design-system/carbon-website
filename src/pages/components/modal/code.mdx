...
This section explains usage with our React variant. For usage with other frameworks, please follow a link in component live demo below.
...
`<Modal>` allows you to use its bespoke set of contents:
...
`children` prop maps to the modal body content. You can also use `modalLabel`, `modalHeading`, `secondaryButtonText` and `primaryButtonText` props to change the corresponding text.
