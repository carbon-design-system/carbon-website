...
<InlineNotification>
  ...
  and use <code>bx--modal-content__regular-content</code> class to apply the 20% padding
...
<InlineNotification>
  <h5>Scrolling behavior</h5>
  In case where the largest modal size cannot fit your modal body content in,
  Carbon design specifies to have a "visual fade" look at the end of the modal body
  area to indicate there is additional content out of view. You can set
  <code>hasScrollingContent</code> prop to <code>{`<Modal>`}</code>/<code>{`<ModalBody>`}</code>
