...
![tructure and spacing measurements for a passive modal](images/modal-style-1.png)
...
Modals that are 36% width and larger (see [sizes](/components/modal/style#sizes)) have a `margin-right: 20%` (margin percentage is based off the width of the modal window). If the modal is smaller than 36% then it has a fixed `margin-right: 16px/1rem`. Body copy, including titles, in a modal alway follows the 20% margin-right rule. However, inputs and other components may still expanded the full width of modal window.

<Row>
<Column colLg={12}>

![tructure and spacing measurements for a passive modal](images/modal-style-3.png)
