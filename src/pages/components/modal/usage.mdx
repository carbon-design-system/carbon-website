...
The task completion action should take place immediately. If a longer loading period is needed then a [loading spinner](/components/loading/usage) and overlay should appear on top of the modal body area with content disabled. The primary action button should be disabled while loading is in progress.
