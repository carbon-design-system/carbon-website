...
When placing buttons, Cancel is always the outmost left button option and the primary action is always the outmost right button. There should only ever be one primary action per dialog. Dialog buttons are always full bleed and attached to the bottom of a dialog. When one or three buttons are used they are aligned to the right edge of the modal with a gap remaining on the left. No more than three buttons should be used in the footer of a modal.
...
For short, direct messages the title can include the whole message to add visual clairty to an otherwise repeativite title and body message. When using this style,no other body copy may be included.
...
There are three actions in the progress modal footer: Cancel, Previous, Next. Each button’s width is 25% of the dialog window and are full bleed. Previous and Next are grouped together and placed on the right half of the dialog, with Previous as a secondary button and Next as a primary button. In the last step of the sequence, the Next button label should change to reflect the final action. The Cancel button is aligned to left side of the dialog and uses a ghost button.
