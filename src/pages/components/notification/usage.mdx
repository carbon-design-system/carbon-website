...
Toast notifications can be actionable and allow for interactive elements. Only
one tertiary action button is allowed within toast notifications. Action buttons
should clearly state the user action. This action frequently takes users to a
flow or page related to the message, where they can resolve the notification. If
multiple actions are required, consider using a
[dialog](https://carbondesignsystem.com/patterns/dialog-pattern/).
...
<Caption>
  Example of a toast notification with action button (left) and a toast
  notification with time stamp (right)
</Caption>
...
Toasts can be automatically dismissed or dismissed by the user. If the toast
includes an action button, then the notification should remain on screen until
the user dismisses it. The user then has enough time to interact with the button
without the toast closing too soon. If a toast is not actionable, it can dismiss
automatically after five seconds on the screen or be dismissed by the user. They
can also include a close button so users can dismiss them sooner. Is a toast
notification does dismiss automatically, users must be able to access them
elsewhere after the toast disappears. This allows them to be accessible for
users who need more time reading the notification or who may want to refer back
to the notification. All toast notifications cover content on the screen so they
should always be easily dismissed.
...
Carbon supports actions within notifications for inline and toast variants. With
actionable notifications, only one action is allowed per notification. This
action frequently takes users to a flow or page related to the message, where
they can resolve the notification. Consider using a notification center where a
user can revisit and act on past notifications.

<Row>
<Column colLg={8}>

![Actionable inline and toast notifications](images/08_toast-action-608.png)
