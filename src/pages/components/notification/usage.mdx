...
6. **Toast notification action (optional):** Link included in the message that allows users to address the notification.
7. **Time stamp (optional):** Shows the time the toast notification was sent.
...
Toast notifications can include a link at the end of their body content. This link should be short (one or two words) and it should navigate users to a page or modal where they can take action to address the notification or find further information. Because toast notifications automatically dismiss, there must be alternative routes to navigate to the link destination.

Currently, Carbon toast notifications are not fully accessible when they include interactive content. The Carbon team is aware of the accessibility concerns and is working on solving them. In the meantime, we recommend that, when possible, interactive content is not included in toast notifications.
