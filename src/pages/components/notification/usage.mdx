...
For more context on when to use each notification type, including modals, refer to the [notifications pattern](/patterns/notification-pattern/). Carbon only supports inline, toast, and modal notification types, although some product teams also support banners and notification centers.
