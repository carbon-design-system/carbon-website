...
![a numeric input with helper text to enter values between 30 and 70, in increments of 5](images/number-input-accessibility-2.png)
...
Carbon’s default number input component is the same width as the default text input, yet numbers typically take up much less space. The + and - buttons are the primary user cue for a number input; however, people using magnification will not see this cue if the buttons and label are not visible in their viewport.  Where possible, reduce the width of the number field to improve its accessibility.
