...
Number inputs are similar to text inputs, but used to specify a numeric value.
Number input lets users enter a numeric value and incrementally increase or
...
- Enabling the user to type a number in the field
...
- It is best practice not to use number input if large value changes are
  expected. For examples, when the value sets from 1 to 30, number input is not
  a great option. In this case, use
...
2. **Numeric value**: The value changes when users entered into the field.
3. **Helper text** (optional, except for error and warning): Assistive text that
   can provide additional aid or context to the user. Often used to explain the
   correct data format.
...
The width varies in sizes based on content, layout, and design.
...
- Always include a label so that users know what to complete the field
...
![Do include a label](images/number-input-usage-4.png)
  ...
  <DoDont type="dont" caption="Do not hide label that helps user understand the context of number input">
...
- Indicate the input value if it has a maximum or minimum
- Helper text is an optional, but required for warning and error states
...
| _Hover_     | When a user is hovering over the value in the number input field or the button controls.                                                                                                                                                                            |
| _Focus_     | When a user presses `Tab` to or clicks on the number input, the field or controlls become focused, indicating the user has successfully navigated to the component.                                                                                                 |
