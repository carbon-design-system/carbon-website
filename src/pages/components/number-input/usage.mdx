...
Number input has a modification that takes on the [AI visual styling]() when the
[AI slug]() is present in the input. The AI variant of number input functions
the same as the normal version except with the additional of the AI slug which
