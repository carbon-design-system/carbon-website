...
<AnchorLink>No tip</AnchorLink>
<AnchorLink>Caret tip</AnchorLink>
<AnchorLink>Tab tip</AnchorLink>
...
1. **Trigger button:** An interactive element that triggers the popover to open
   on click, hover, or focus.
...
![Do keep the width size of a popover within four columns.](popover-usage-4-do.png)
...
Popovers without a tip are typically used when the trigger button has a visually
defined down state.

### Placement

Upon opening, popover directions are set to `auto` by default and can detect
browser edges so they can be properly placed in view without cutoff. Popovers
can also specify distinct directions. Direction options vary depending on the
popover variant.

When a popover does not have a tip, the trigger button should be flush to the
side of the popover and can open at the top, bottom, left, or right directions.
...
![Do flush align the popover edge with the trigger button.](popover-usage-10-do.png)
...
center with eachother and can open at the top, bottom, center, left, or right
directions.
...
Similarly like no tip popovers, a trigger button paired with a caret tip popover
...
![Do align the caret tip center with the trigger button.](popover-usage-14-do.png)
...
Popovers can have a tab tips when the popover is connected to a toolbar or
header area.
...
![Do top align the tab tip with the layer behind it to keep clean lines between the popover and the layer underneath.](popover-usage-17-do.png)
