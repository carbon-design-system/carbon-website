...
4. **Helper text (optional):** Assistive text to show additional information
   about the process taking place. This is not optional for the error state. For
   determinate, the information is usually values of number or percentage.
   Sometimes, a fraction or ratio may be shown. These values are not applicable
...
- Labels should never change as the progress bar is loading. Consider the label change when it reaches 100% to either success or error state. This helps avoid the confusion users may have when losing the initial context of the progress bar. For example, if the progress bar is in active state with a label called 'Download usage data'. When it finishes downloading and the label changes to 'Completed', the users may find it confusing with this subtle change in information or may not remember the inital context of this progress bar. 
...
<Caption>
  The example shows the progress bar is active and seperated from other interactive elements.
</Caption>
...
- The loading component is implemented using the ARIA 1.1 progressbar pattern.
  For determinate progress bar, it outputs an ARIA `role="progressbar"` and uses `aria-valuemin`,
