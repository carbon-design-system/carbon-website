...
- If the process takes less than 5 seconds to load, use the
  [loading](/components/loading/usage) spinner instead
...
| [Indeterminate](/components/progress-bar/usage#indeterminate-progress-bar) | Use indeterminate progress bars where there is unclear information about the progression — they convey that the user's request, action, or data is being processed at the moment without indicating how long the activity will take. |
...
Once triggered, the bar indicator will move either back and forth or continuosly from left to right, while data is being gathered during the first stages of the load. In this scenario, a percentage value would not be present. This bar may change to a determinate indicator if enough information is gathered to calculate the load completion. The final determinate stage of the load is used to visually indicate the completion of the task to the user. 
...
4. **Helper text (optional):** Assistive text to show additional information
   about the process taking place. Not optional for an error. For determinate,
   the text is usually a percentage or numeric value showing progression. Sometimes, a
   fraction or ratio may be shown. These values are not applicable when a
...
Labels should always be placed above the progress bar for readability, and helper text below or beside. Avoid placing text inside the progress bar indicator or track. This visual could cause clutter and inconsistency in text
...
- Label text should never change as the progress bar is loading. It also does not need to be updated when the process ends, since the success or error information is provided by the bar indicator color and an icon, along with any helper text. 
...
The states are indicated by the color of the loading bar, along with an icon for error and success.
...
The success loading state, conveyed by a green progress bar and checkmark icon, indicates that the action completed successfully.
...
<Caption>
  ...
  confirmation inside the image tile. Another upload is initiated. The progress
  bar shows the upload success and then is automatically dismissed.
</Caption>
...
should appear. As soon as a process fails, the indicator bar's progress spans full width, turns red, and remains
visible. A failed icon is also shown.
...
<Row>
  <Column colLg="8">
    ...
    <Caption>
      The example shows the progress bar is automatically dismissed once it’s
      done uploading.
    </Caption>
...
- [Loading: skeleton states](/patterns/loading-pattern/#skeleton-states)
