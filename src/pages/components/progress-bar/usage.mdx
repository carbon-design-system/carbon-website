...
- If an activity displays a loading spinner on one screen, it should not show
...
When it is neccesary to show the percentage and the descriptive text, the
developers will have to calculate the percentage on their side with the value:
...
<Caption>
  Indeterminate progress bars move along a fixed track continually until the
  process is complete.
</Caption>
...
![Do place text inline with the progress bar](images/progress-bar-usage-12.png)
  ...
  <DoDont type="dont" caption="Don’t embed text into the progress bar.">
...
but just stops. For example, if a process fails at 75%, then the red line would
fill 75% of the total track width.
...
- Only hide the label when it is absolute clear to the user which process the
  ...
  appropriate label which will be ready by screen readers.
