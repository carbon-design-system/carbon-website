...
1. **Label:** The text describes the process that the progress bar refers to.
   ...
   accessible.
2. **Helper text (optional for determinant):** Assistive text to show additional
   information about the process taking place. For determinate, the text is
   usually a percentage, fraction, ratio, or numeric value showing progression.
   Such values are not applicable when a progress bar is indeterminate. A
   progress bar that reaches an error state must convey the error in the helper
   text.
3. **Track:** The static gray area that the bar indicator moves on top of acts
   as a fixed visual reference of what the total length and duration of the
   process could be.
4. **Bar indicator:** Indicates how much the process has progressed. Changes
