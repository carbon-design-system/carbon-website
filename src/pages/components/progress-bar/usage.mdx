...
- Labels should never change as the progress bar is loading. It's best to keep the label unchanged when it reaches 100% to either success or error state. This avoids the confusion users may have when losing the initial context of the progress bar. Use helper text to clarify the state if neccessary.  
