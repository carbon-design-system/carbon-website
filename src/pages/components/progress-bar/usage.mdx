...
- Keep the label short and concise by limiting it to a few words or a single
  line of text.
- Labels should never change as the progress bar is loading. Consider the label
  change when it reaches 100% to either success or error state. This helps avoid
  the confusion users may have when losing the initial context of the progress
  bar. For example, if the progress bar is in active state with a label called
  'Download usage data'. When it finishes downloading and the label changes to
  'Completed', the users may find it confusing with this subtle change in
  information or may not remember the inital context of this progress bar.
