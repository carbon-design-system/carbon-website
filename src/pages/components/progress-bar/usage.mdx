...
how long a process will take, or a request is being excuted.
...
- When the process can be described with quantitative information, for example
  as a percentage
...
- If the process takes less than 10 seconds to load, use
  [loading spinners](/components/loading/usage) instead

#### Loading, inline loading, or progress bar?

- If an activity displays a loading spinners on one screen, it should not show
  the progress bar elsewhere in the design.
- Both progress bars and circles can show either determinate or indeterminate
  progress. The given area should help determine if a progress bar or circle is
  best for that space. Progress bars are preferred in vertically narrow areas
  (tables, cards, etc.) while progress circles are ideal for full page loading
  or in very small areas.
...
<Caption>
  Indeterminate progress bars move along a fixed track, growing and shrinking in
  size.
</Caption>
...
decrease in value. The entire animation moves from the left to the right – with
no back and forth motion.

Use a determinate progress bar when progress can be calculated against a
...
the data source and API configuration. In these cases, indeterminate progress
bars can be used.

#### Local Processing

Local processing refers to manipulations or operations done on the local device
that does not require communication with an external entity such as a server.
This can include currency exchange, file generation, data reformatting, or
similar operations. Depending on the operation, time estimated or cycles to
complete processing could be used to populate a determinate progress bar.
Otherwise, an indeterminate progress bar would be best.

## Formatting

Determinate and indeterminate progress bars both have the labels and optional
helper text. However, the inderterminate can’t provide the exact, quantiative
...
1. **Label:** The text describes the process that the progress bar refers to.
   Label can be visually hidden, but requires to be defined to be accessible.
...
The progress bar is offered in two different sizes—default (8px) and small
(4px). The default progress bar height is typically used when there is large
amounts of space on a page. The small progress bar height is commonly used when
space is restricted and can be placed within cards, data table, or side panels.
...
placed close to the progres bar, it’s up to designers to provide the best
...
<Caption>A few common examples of text alignmnet for progress bars</Caption>
...
progress bar for readibility. Avoid placing text inside or centering text with
...
- Helper text is used to show additional information about the process taking
  place.
- Common usecases of helper text for a progress bar can either be a generic
...
should appear. The error loading state indicates that the action did not
successfully complete. If an error occurs, an inline notification or error
handling within the form should appear. As soon as a process fails, the current
visual progress remains but just stops. For example, if a process fails at 75%,
then the red line would fill 75% of the total track width.
...
still interact with other elements on the page. When the progress bar is
finished processing it can either remain persistant on the page for confirmation
or automatically dismiss itself depending on the usecase.
...
The loading component is implemented using the ARIA 1.1 progressbar pattern. It
outputs an ARIA role="progressbar" and uses `aria-valuemin`, `aria-value-max`,
and `aria-valuenow` to convey the loaded percentage to screen reader users.

If is neccesary to show the percentage and the descriptive text, the developers
will have to calculate the percentage on their side with the value
`${(value/max*100).toFixed(2)}% (${value} of ${max} items)`.
...
Accessible Rich, Internet Applications Working Group, [“progressbar” role],
(W3C, 2017) Katie Sherwin,
[Progress Indicators Make a Slow System Less Insufferable](https://www.nngroup.com/articles/progress-indicators/)
(Nielsen Norman Group, 2001) Jakob Nielsen,
