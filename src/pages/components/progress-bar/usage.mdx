...
- When the load time is expected to display expanded information, use [skeleton states](/patterns/loading-pattern/#skeleton-states) instead
...
right (Carbon reserves back and forth motions for [skeletons](/patterns/loading-pattern/#skeleton-states)), while data is being gathered during the first stages of the load. In this scenario, a
...
1. **Label:** The text describes the process that the progress bar refers to.
   ...
   accessible.
2. **Helper text (optional for determinant):** Assistive text to
...
3. **Track:** The static gray area that the bar indicator
   moves on top of acts as a fixed visual reference of what the total length
   and duration of the process could be.
4. **Bar indicator:** Indicates how much the process has progressed. Changes color based on status.
...
The success loading state, conveyed by a full-width green progress bar and checkmark icon,
