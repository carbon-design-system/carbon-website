...
endpoints." A label my use one word if it is universally understood, such as
...
![Example of progress indicator with overflow content](images/progress-indicator-usage-6.png)
...
started**, **error**, **disabled**, **focus**, and **hover**. View the
...
If the progress indicator is interactive, users can trigger a state change by
clicking on the step label.

#### Keyboard

The first step should be selected by default. Users can navigate between steps
by pressing right or left arrow keys. For additional keyboard interactions, see
