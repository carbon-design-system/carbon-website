...
Only one radio button can be selected at a time. When a user chooses a new
...
VoiceOver: Users can navigate between radio buttons by pressing the `Up` and `Down`
arrow keys.
