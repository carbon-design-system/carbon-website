...
for focus, disabled, and read-only. When deciding whether to use a disabled or
read-only state for radio buttons, see our
...
![Examples of focus, disabled, and read-only states](images/radio-button-usage-8.png)
