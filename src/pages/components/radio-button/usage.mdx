...
<DoDontRow>
  <DoDont
    colMd={4}
    colLg={4}
    caption="Do use checkboxes when multiple items can be
    selected.">
    ![Do use checkboxes when multiple items can be
    selected.](images/radio-button-usage-2-do.png)
  </DoDont>
  <DoDont
    colMd={4}
    colLg={4}
    type="dont"
    caption="Don't use radio buttons when multiple items can be
    selected.">
    ![Don't use radio buttons when multiple items can be
    selected.](images/radio-button-usage-2-dont.png)
  </DoDont>
</DoDontRow>
...
for easier reading. For languages that read left to right, it's acceptable to
place labels to the left of the inputs.
...
changing settings in a menu or selecting a singular item in a data table. Radio
buttons in a form should be placed at least 32px (`layout-03`) below or before
the next component. Spacing of 24px (`layout-02`) or 16px (`layout-01`) can also
be used when space is more restricted or if the form is more complex.
...
- Text should wrap beneath the radio button so the control and label are top
  ...
  Do let text wrap beneath the radio button so the control and label are top
  aligned. Do not vertically center wrapped text with the radio button.
...
Only one radio button should be selected at a time. When a user chooses a new
item, the previous choice is automatically deselected. In addition to unselected
and selected states, radio buttons also have states for focus and disabled.
