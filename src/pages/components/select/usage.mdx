...
<Caption>
  Example of a select list of options for data submission versus a dropdown list
  of actional options.
</Caption>
...
1. **Labels:** Text that informs the user what to expect in the list of dropdown
   options.
2. **Default option:** An empty option or a frequent option that users would
   choose from a select list.
...
**warning**, **disabled**, and **read-only**.
...
![Select keyboard interaction](images/select-usage-keyboard-interaction-fixed.png)
...
By default, the selected option is usually an empty option, the first item in
alphabetical order, or a frequent option that users would choose from a select
list.
