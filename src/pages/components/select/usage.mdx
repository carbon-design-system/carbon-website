...
- A select presents a list of options from which the users can select only one
  It works best in forms when users choose an option from
...
<Caption>
  Example of a dropdown list of actional options
  versus a select list of options for data submission.
</Caption>
...
Avoid having multiple lines of text per item in a select menu. The menu items should be
...
- Users can open the dropdown menu by pressing `Space`, `Enter`, the `Down arrow`, or
  the `Up arrow`.
