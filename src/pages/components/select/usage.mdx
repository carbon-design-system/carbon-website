...
_Select_ is a type of input that is used in forms, where a user is submitting
data and chooses one option from a list.
...
_Small selects_ are commonly used in [data tables](/components/data-table/code).
...
[forms](/components/form/code) usage page.
