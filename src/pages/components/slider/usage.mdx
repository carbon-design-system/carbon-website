...
With the focus on the slider itself, users can also increment the value using the arrow keys. The arrow keys in combination with the `Shift` key can alter the slider by the value of the **stepMultiplier**. _Example: With `setpMultipler` set at `10`, typing_ `Shift` + → _changes the value from 60 to 70, increasing the inputted value by 10 units._
