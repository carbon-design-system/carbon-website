...
Users can enter an exact numeric value into the text input field. While in the text field, using the ↑ ↓ ← → arrow keys increments the value in the text input. Either method moves the slider handle to the corresponding position.

Users can also increment the value with the focus on the slider itself using the arrow keys. The arrow keys in combination with the `Shift` key can alter the value by the value of the **stepMultiplier**. _Example: With `setpMultipler` set at `10` Typing_ `Shift` + → _changes the value from 60 to 70, increasing the inputted value by 10 units._
