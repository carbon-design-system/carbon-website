...
the slider by the value of the `stepMultiplier`. For example, set the
`stepMultiplier` at `10`, and type `Shift` + `Right` arrow key to change the
value from 60 to 70, increasing the inputted value by 10 units.
