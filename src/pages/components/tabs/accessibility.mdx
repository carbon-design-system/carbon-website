...
Tabs take at least two tabstops, one for the tablist, one for the tabpanel.
Tabbing away from the tablist, focus will either go to the first operable
...
to scroll sideways within the list. For keyboard users, the navigation does not
change. The user presses the `Left` or `Right` arrow keys, which moves the focus
