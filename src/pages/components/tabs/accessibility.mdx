...
`Left` or `Right` arrow keys, which moves the focus to the next tab item and
...
<Caption>
  Arrows keys move between tabs in the tablist (including wrapping from last
  back to first). The tablist display scrolls automatically to keep the focused
  tab visible.
</Caption>
