...
Each tab label describes its content and sets user expectations. Labels are concise and use one to two words maximum. Keep in mind that at mobile widths, the character length of a label will impact the experience. Icons are not permitted in tab labels.

#### Number of tabs

A maximum of six tabs is recommended for most scenarios. This is to maintain an uncluttered UI and reduce cognitive load for users. If more than six tabs are needed then consider other navigation patterns, such as a [side-nav](/components/UI-shell-left-panel/code).
