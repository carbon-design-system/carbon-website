...
When the tag's title is too long to fit within the container's width, the title
will be truncated with an ellipsis. The full title is disclosed in a tooltip on
focus.

<Row>
<Column colLg={8}>

![An operational tag in the tab order, disclosing related tags.](images/tag-accessibility-overflow.png)
...
- Do not nest buttons within the operational tag. Consider using
  `PolymorphicProps` to change an element tag to avoid nesting buttons.
