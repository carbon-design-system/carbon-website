...
Selectable tags are in the tab order and focus is shown around each selectable
tag. Pressing `Enter` or `Space` toggles the selection on and off.
...
Operational tags are in the tab order and focus is shown around each operational
tag. Pressing `Enter` or `Space` will disclose additional related tags.
