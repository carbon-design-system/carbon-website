...
| Variant                              | Purpose                                                                                                                                                                                             |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Read-only tag](#read-only-tags)     | Tags that have no interactive functionality and are commonly used for categorizing and labeling.                                                                                                    |
| [Dismissible tag](#dismissible-tags) | Tags that can be dismissed, closed, or removed. These tags are typically used for filtering and creating user-generated content.                                                                    |
| [Selectable tag](#selectable-tags)   | Tags that can be selected or deselected and are frequently used as selections that filter data in the context of a page. They also can be used in chat flows to make decisions to advance the chat. |
| [Operational tag](#operational-tags) | When interacted with, these tags can disclose additional or overflow tags, like in a toggletip, popover, modal, or breadcrumb detail view.                                                          |
...
There are three different tag sizes— large, medium, and small. Use large tags
when they are used as a primary task of the page or focal point, when you have
more screen real estate at your disposal, or if the tag lives near other
components that are also 32px in height. The medium tag size is the carbon
default size and is used in most cases. Use small tags in condensed or inline
spaces.

<Row>
<Column colLg={8}>

![Tags shown in large, medium, and small sizes.](images/tag-usage-sizing.png)

</Column>
</Row>

<Caption>Tags shown in large, medium, and small sizes.</Caption>
...
When the tag's title is too long to fit within the available space of its
container, the title can be truncated with an ellipsis. The full title will be
disclosed in a tooltip on hover. Truncation should be set at the title's start,
middle, or end, depending on what is best for the given use case.

<Row>
<Column colLg={8}>

![A tag with a truncated title, disclosing the full title on hover in a tooltip.](images/tag-usage-overflow.png)

</Column>
</Row>

<Caption>
  A tag with a truncated title, disclosing the full title on hover in a tooltip.
</Caption>

Avoid letting long tag titles wrap to multiple lines within the tag container.
This can construe the shape of the tag and the integrity of a traditional tag
shape, which should be compact in form. It can also create misalignment with
other tags if they are placed within a group.

<DoDontRow>
  <DoDont type="do" caption="Do use a tooltip to show the full title length of overflow tag text.">

![Do use a tooltip to show the full length of overflow tag title text.](images/tag-usage-overflow-do.png)
...
- **Dismissible and operational tags:** enabled, hover, focus, on click,
  disabled, and skeleton
- **Selectable tags:** enabled, hover, focus, selected, disabled, and skeleton

For more information on tag states, see the style tab.
...
![Do wrap tags in a group to a few multiple lines.](images/tag-usage-selectable-wrap-do.png)

  </DoDont>
  <DoDont type="dont" caption="Do not wrap tags in a group of more than five lines.">

![Do not wrap tags in a group of more than five lines.](images/tag-usage-selectable-wrap-dont.png)
...
disclosed in a toggletip, popover, or breadcrumb detail view, for example. Do
not use tags as links that direct you to an entirely different page or launch
you from a current experience to a separate tab.
...
[component tokens](https://carbondesignsystem.com/guidelines/color/tokens/). It
is recommended to use multiple colors to denote different categories or labels.
Selectable tags are unavailable in these colors; instead, they use
[core tokens](https://carbondesignsystem.com/guidelines/color/overview/#core-tokens).
