...
[Experimental tags](https://react.carbondesignsystem.com/?path=/docs/experimental-unstable-interactivetag--overview) for dismissible, selectable, and operational functionality are now available. Read-only tags and the filter prop remain stable in the system. Although we are not deprecating the filter prop in the read-only tag, we encourage all design teams to use the experimental "dismissible" tag in their products moving forward.
