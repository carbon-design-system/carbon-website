...
<AnchorLink>AI Presenece</AnchorLink>
...
Text input and text area have a modification that takes on the
[AI visual styling]() when the [AI slug]() is present in the input. The AI
variant functions the same as the normal version except with the additional of
the AI slug which is both a visual indicator and the trigger for the
[explainability popover]().

[Learn more about designing for AI]()
