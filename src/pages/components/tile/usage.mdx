...
When using a call-to-action (CTA) within a tile, use a [secondary button](/components/button/code). Primary buttons should be reserved for the most important action a user can take on the page.
...
Read-only tiles are used to display information to the user, such as features or services offered. Read-only tiles are often seen on marketing pages to promote content. These tiles can have internal calls-to-action (CTAs), such as a [button](/components/button/code) or a [link](/components/link/code).

<Row>
<Column colLg={8}>

![Example image of read-only tiles.](images/tile-usage-1.jpg)
...
![Example image of clickable tiles.](images/tile-usage-2.jpg)
...
Selectable tiles work like a [radio button](/components/radio-button/code), where the entire tile is a click target. Selectable tiles may contain internal CTAs (like links to docs) if the internal CTA is given its own click target. Selectable tiles work well for presenting options to a user in a structured manner, such as a set of pricing plans.

<Row>
<Column colLg={8}>

![Example image of selectable tiles.](images/tile-usage-3.jpg)
...
![Example image of expandable tiles.](images/tile-usage-4.jpg)
