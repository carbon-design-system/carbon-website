...
The image below should show the icons in blue color instead of gray. We will
update this soon.
...
| _Disabled_ | When the user is not allowed to interact with the tile due to either permissions, dependencies, or pre-requisites. The disabled state completely removes the interactive function from a component. The styling is not subject to WCAG contrast compliance. |

<Row>
<Column colLg={8}>

![Clickable tiles vs disabled tiles](images/tile-usage-19.png)

<Caption>
  Clickable tile's disabled state switches the default icon to an error icon.
</Caption>
...
![Clickable, expandable, and selectable tiles are clickable anywhere within the tile.](images/tile-usage-20.png)
...
![Base tiles are static, except buttons or links .](images/tile-usage-21.png)
...
![Expandable tiles click targets](images/tile-usage-22.png)
