...
A toggletip has two states: open and closed. The open state occurs when a user
clicks on or enter the trigger button, while the closed state means the
toggletip is hidden.
...
| ----- | -------------- | -------- | --------------- |
| Open  | trigger button | svg      | `$icon-primary` |
| Open  | trigger button | border   | `$focus`        |

<Row>
<Column colLg={12}>

![Toggletip closed and open states](images/toggle-tip-style-1.png)
