...
you need to expose information that a user needs to interact with.
...
- Use for menus such as profile or settings
...
1. **Trigger element:** Usually a button but as long as trigger is interactive,
   it can visually change its shape and size depending on the use case.
...
toggletip pop over may be positioned top, bottom, left, or right to the UI
trigger.
...
Placement of popover in relation to the trigger depends on the type of popover
...
scrolling is usually not needed. If scrolling is needed, in a drop down like
