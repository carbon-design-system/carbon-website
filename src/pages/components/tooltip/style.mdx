...
| ------------ | --------------------------------------- | ---------------- | --------------------- |
| Open         | container (all components with tooltip) | background-color | `$background-inverse` |
| Open         | text (all components with tooltip)      | text-color       | `$text-inverse`       |
| Open (hover) | trigger (definition tooltip)            | border           | `$border-interactive` |
| Open (focus) | label and border (definition tooltip)   | border           | `$focus`              |
...
The tooltip is designed for both multi-line text, with 16px padding on all sides
for better spacing, and single-line text, which is more compact since it
typically contains only a few words and doesn’t wrap.
