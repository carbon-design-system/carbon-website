...
on creating the keyboard interaction visuals.
...
Choose the first if designers need to include annotations in their wireframes to make the component accessible.

Example 1: Design annotations are needed for specific instances shown below, but for the
standard (component name) component, Carbon already incorporates accessibility.

Choose the second if the component is accessible without additional design decisions. If this is the case, the Design recommendations section should also be removed and the anchor link removed.

Example 2: No accessibility annotations are needed for (component name)s, but keep these considerations in
