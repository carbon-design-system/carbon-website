---
title: Chart types
description: Overview of all chart types available.
---

import OverviewCard from 'components/OverviewCard';

<PageDescription>

Start by identifying the purpose of the visualization and then choose the
appropriate chart type.

</PageDescription>

## Comparisons

<Row className="overview-card-group">
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Simple bar (vertical)"
    href="/data-visualization/simple-charts#simple-bar-(discrete)"
    >

![GATSBY_EMPTY_ALT](images/bar.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Grouped bar (vertical)"
    href="/data-visualization/simple-charts#grouped-bar-(discrete)"
    >

![GATSBY_EMPTY_ALT](images/bargrouped.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Bubble"
    href="/data-visualization/simple-charts#bubble"
    >

![GATSBY_EMPTY_ALT](images/bubble.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Radar"
	href="/data-visualization/simple-charts#radar"
    >

![GATSBY_EMPTY_ALT](images/radar.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Simple bar (horizontal)"
    href="/data-visualization/simple-charts#simple-horizontal-bar-(discrete)"
    >

![GATSBY_EMPTY_ALT](images/barhoriz.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Grouped bar (horizontal)"
    href="/data-visualization/simple-charts#grouped-horizontal-bar-(discrete)"
    >

![GATSBY_EMPTY_ALT](images/grouped-bar-horizontal.svg)

</OverviewCard>
</Column>
</Row>

## Trends

<Row className="overview-card-group">
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Line"
    href="/data-visualization/simple-charts#line"
    >

![GATSBY_EMPTY_ALT](images/line.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Area"
    href="/data-visualization/simple-charts#area"
    >

![GATSBY_EMPTY_ALT](images/area.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Histogram"
	disabled
    >

![GATSBY_EMPTY_ALT](images/histogram.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Stream"
	disabled
    >

![GATSBY_EMPTY_ALT](images/stream.svg)

</OverviewCard>
</Column>
</Row>

## Part-to-whole

<Row className="overview-card-group">
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Donut"
    href="/data-visualization/simple-charts#donut"
    >

![GATSBY_EMPTY_ALT](images/donut.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Pie"
    href="/data-visualization/simple-charts#pie"
    >

![GATSBY_EMPTY_ALT](images/pie.svg)

</OverviewCard>
</Column>

<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Stacked bar (horizontal)"
    href="/data-visualization/simple-charts/#stacked-horizontal-bar-(discrete)"
    >

![GATSBY_EMPTY_ALT](images/stacked-bar-horizontal.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Stacked bar (vertical)"
    href="/data-visualization/simple-charts/#stacked-bar-(discrete)"
    >

![GATSBY_EMPTY_ALT](images/barstack.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Stacked area"
    href="/data-visualization/simple-charts/#stacked-area-chart"
    >

![GATSBY_EMPTY_ALT](images/areastack.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
<OverviewCard
   title="Meter"
   href="/data-visualization/simple-charts/#meter"
   >

![GATSBY_EMPTY_ALT](images/meter.svg)

</OverviewCard>
</Column>

<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
<OverviewCard
   title="Gauge"
   href="/data-visualization/simple-charts/#gauge"
   >

![GATSBY_EMPTY_ALT](images/gauge.svg)

</OverviewCard>
</Column>

<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm>
 <OverviewCard
    title="Tree map"
    href="/data-visualization/complex-charts#tree-maps"
    >

![GATSBY_EMPTY_ALT](images/treemap.svg)

</OverviewCard>
</Column>
</Row>

## Correlations

<Row className="overview-card-group">
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm>
 <OverviewCard
    title="Scatter"
    href="/data-visualization/simple-charts#scatter"
    >

![GATSBY_EMPTY_ALT](images/scatter.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Heat map"
    href="/data-visualization/complex-charts#heat-maps"	       
    tag="Design only"
    >

![GATSBY_EMPTY_ALT](images/heatmap.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Parallel coordinates"
    href="/data-visualization/complex-charts#parallel-coordinates"	       
    tag="Design only"
    >

![GATSBY_EMPTY_ALT](images/parallel.svg)

</OverviewCard>
</Column>
</Row>

## Connections

<Row className="overview-card-group">
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Alluvial diagram"
    href="/data-visualization/complex-charts#alluvialsankey-diagrams"
    tag="Design only"
    >

![GATSBY_EMPTY_ALT](images/alluvial.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Network diagram"
    href="/data-visualization/complex-charts#network-diagrams"
    tag="Design only"
    >

![GATSBY_EMPTY_ALT](images/treediagram.svg)

</OverviewCard>
</Column>
</Row>

## Geospatial: overlays

<Row className="overview-card-group">
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Choropleth map"
    href="/data-visualization/complex-charts#geographic-overlays"
    tag="Design only"
    >

![GATSBY_EMPTY_ALT](images/geo-choropleth.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Proportional symbol"
    href="/data-visualization/complex-charts#proportional-symbol-map"
    tag="Design only"
    >

![GATSBY_EMPTY_ALT](images/geo-proportion.svg)

</OverviewCard>
</Column>
<Column  colMax={3} colXl={3} colLg={4} colMd={4} colSm={2} noGutterSm >
 <OverviewCard
    title="Connecting lines"
    href="/data-visualization/complex-charts#connection-map"
    tag="Design only"
    >

![GATSBY_EMPTY_ALT](images/geo-connection.svg)

</OverviewCard>
</Column>
</Row>
