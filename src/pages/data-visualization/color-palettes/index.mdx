...
Graidents are good for highlighting extremes in a range of values. Use gradient on single category visualizations only if needed. Gradients should not be used to represent any meaningful progression or divergence, so it should never be used in place of a sequential palette. Use gradients on single category visualizations. Multiple gradients are easily inaccessible and is discouraged in our system.
