...
Hovering over the legend of one category lowers the opacity of all other categories charts to 30%.
...
When all categories are selected, checkmarks in legends disappear, and the legends reset to default state.
...
Please note that hiding legends is discouraged in data visualization (unless only one category of data is displayed). This design is for mobile situations where having legends at a glance is less essential. In general, hiding legends reduces the clarity of the visualization and is inaccessible.
