...
### Access request screen captures

After clicking the link above, sign in with your IBMid (w3id for IBMers) at the
screen below:

![Login to IBM Cloud](./images/log_in.png)

After signing in with your IBMid, if you are prompted to request access to
Carbon MCP, you can complete the brief request form.

![Carbon MCP access request](./images/submit_request.png)

Once your access request has been approved, you will receive an email to
activate your credentials. You'll then click the _"Continue to Carbon MCP
sign-in"_ button.

![Carbon MCP invitation email](./images/invitation_email.png)

You will then be provided with your authentication credentials, as shown below.
Once there, copy/paste the `Authorization code` and `Session ID` or
`Auth headers JSON` into your MCP client config.

![Carbon MCP auth credentials](./images/auth_creds.png)
...
In Bob, select the icon in the upper right for the Bob Marketplace.

<Row>
<Column colMd={8} colLg={9}>

![Bob Marketplace](./images/carbon-mcp-bob-marketplace-step1.png)
...
Select the **Modes** tab, then search for **"Carbon React"** in the text entry
field. Click the **Install** button.
...
"https://mcp.carbondesignsystem.com/mcp", "disabled": false, "timeout": 600,
"headers": { "Authorization": "Bearer <TOKEN>", "X-MCP-Session": "<SESSION>" },
"tools": ["*"], "alwaysAllow": ["code_search", "docs_search"] } } }
```
...
```markdown
{ "mcpServers": { "carbon-mcp": { "type": "streamable-http", "url":
"https://mcp.carbondesignsystem.com/mcp", "systemPrompt":
"<CARBON_MCP_SYSTEM_PROMPT>", "disabled": false, "timeout": 600, "headers": {
"Authorization": "Bearer <TOKEN>", "X-MCP-Session": "<SESSION>" },
"alwaysAllow": ["code_search", "docs_search"] } } }
```
