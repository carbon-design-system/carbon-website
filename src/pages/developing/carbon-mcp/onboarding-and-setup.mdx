...
- Upon successful authentication, return the the Carbon MCP connector dialogue
  in Figma Make
- Toggle on the "Enable write tools" button
- For the `doc_search` and `code_search` tools, select "Always run" in the
  dropdown menu
- Click the "Back" button, then click "X" to close the Connectors menu
- Start a new Make with Carbon MCP, using this as
  [GUIDELINES.md](/developing/carbon-mcp/files/GUIDELINES.txt) and this as the
  [input user prompt](/developing/carbon-mcp/files/figma-make-user-prompt.txt),
  and select the Anthropic "Opus 4.6." model (or later) for best code generation
  results
