---
title: Developer resources
description:
  Here you'll find Carbon tools, as well as our GitHub repos and Storybooks for
  your framework of choice.
tabs: ['Resources', 'Community']
---

<PageDescription>

Here you'll find Carbon tools, as well as our GitHub repos and Storybooks for
your framework of choice.

</PageDescription>

## Resources

<AnchorLinks>

<AnchorLink>GitHub repos</AnchorLink>
<AnchorLink>Storybooks</AnchorLink>
<AnchorLink>Tutorials</AnchorLink>
<AnchorLink>Tools</AnchorLink>
  <AnchorLink>Carbon Figma and AI tools</AnchorLink>

</AnchorLinks>

## GitHub repos

<Row className="resource-card-group">
  <Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon"
    href="https://github.com/carbon-design-system/carbon"
    >

<MdxIcon name="github" />

  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon React"
    href="https://github.com/carbon-design-system/carbon/tree/main/packages/react"
    >

<MdxIcon name="github" />

  </ResourceCard>
</Column>

<Column colMd={4} colLg={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon Web Components"
    href="https://github.com/carbon-design-system/carbon/tree/main/packages/web-components">
    <MdxIcon name="github" />
  </ResourceCard>
</Column>

<Column colMd={4} colLg={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon Charts core"
    href="https://github.com/carbon-design-system/carbon-charts/tree/master/packages/core">
    <MdxIcon name="github" />
  </ResourceCard>
</Column>

</Row>

## Storybooks

<Row className="resource-card-group">
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon React Storybook"
    href="http://react.carbondesignsystem.com/"
    >

<MdxIcon name="react" />

  </ResourceCard>
</Column>

<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
      subTitle="Carbon Web Components Storybook"
      href="http://web-components.carbondesignsystem.com/"
      >

<MdxIcon name="webcomponents" />

  </ResourceCard>
</Column>

<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
      subTitle="Carbon Charts Website"
      href="https://charts.carbondesignsystem.com/"
      >

![](/images/storybook-addon-carbon-themes.png)

  </ResourceCard>
</Column>

</Row>

## Tutorials

<Row className="resource-card-group">
  <Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      subTitle="Carbon React tutorial"
      href="/developing/react-tutorial/overview/">
      <MdxIcon name="react" />
    </ResourceCard>
  </Column>

{' '}

<Column colMd={4} colLg={4} noGutterSm>
  <ResourceCard
    subTitle="Web components tutorial using Vanilla.js/HTML"
    href="/developing/web-components-tutorial/overview">
    <MdxIcon name="webcomponents" />
  </ResourceCard>
</Column>

</Row>

## Tools

<Row className="resource-card-group">

<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon Storybook Theme Switcher"
    href="https://www.npmjs.com/package/@carbon/storybook-addon-theme"
    >

<MdxIcon name="npm" />

  </ResourceCard>
</Column>

<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Stylelint plugin Carbon tokens"
    href="https://www.npmjs.com/package/stylelint-plugin-carbon-tokens"
    >

<MdxIcon name="npm" />

  </ResourceCard>
</Column>

</Row>

## Carbon Figma and AI tools
Carbon Figma Tools is a suite of plugins built to supercharge IBM product teams by 
bridging the gap between design and development. Powered by the Carbon Design System 
and infused with AI, these tools help you deliver production-ready code faster, with 
greater accuracy and consistency.

<Row className="resource-card-group">
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon Figma and AI tooling"
    href="https://w3.ibm.com/w3publisher/carbon-figma-tooling"
    actionIcon="launch">
![Bee icon](images/ecosystem/bee.svg)

  </ResourceCard>
</Column>
</Row>