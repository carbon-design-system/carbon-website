...
- `@carbon/react` includes components, styles and icons.
...
@use './components/TutorialHeader/tutorial-header.scss';
```
...
Now let's import the icons from our `@carbon/icons-react` elements package,
which is included in our `@carbon/react` exports. In the `TutorialHeader.js`
...
You should now see a styled UI Shell header and a button below it, however the
UI Shell is being rendered in the white theme. Let's import the `Theme`
component styles, so it applies the correct `g100` theme we configured.

In `app.scss` we need to import the theming styles below our component styles.
...
In `index.scss` add these overriding styles. This is needed because the
`Content` component receives some styles using the selector
`.bx—header ~ .bx—content`, but these styles are no longer being applied since
both the header and content are being wrapped by the theme div.
...
You should now see a g100 themed UI Shell header and a button below it.
...
```scss path=src/app.scss
@use './components/TutorialHeader/tutorial-header.scss';
@use './content/LandingPage/landing-page.scss';
@use './content/RepoPage/repo-page.scss';
