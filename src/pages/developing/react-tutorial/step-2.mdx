...
A [preview](https://v11-react-step-3--carbon-tutorial.netlify.com) of what
you'll build:
...
```bash
git fetch upstream
git checkout -b v11-react-step-2 upstream/v11-react-step-2
```
...
the expected `Grid` > `Column` DOM structure. Nested grids are officially supported
with our CSS Grid. If you are using `FlexGrid`, subgrid is not supported.
...
Lastly, we'll fix some grid alignment issues. We'll use one of our breakpoint
mixins for the media queries, like so:
...
We are almost done with the landing page. You may notice a few styles are off. To fix this, we'll update some of the overriding styles in `index.scss`. We already have some overriding styles for `.cds--content`. We need to add `padding: 0;` to this selector and add one more selector for the grid below that:
...
Now in our `RepoPage.js` we'll import our `Grid` components at the top:
...
that are needed by the `Table`, `TableHeader` and `TableRow` components respectively. We
...
```bash
git push origin v11-react-step-2
```
...
`v11-react-step-2` into `base: v11-react-step-2`.
