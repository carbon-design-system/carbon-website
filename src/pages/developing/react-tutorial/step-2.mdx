...
```bash
git fetch upstream
git checkout -b react-step-2 upstream/v11-react-step-2
```
...
In order to use the grid, we need to wrap everything in a `<Grid>` component.
With our CSS Grid, we no longer need rows. We will only be using `<Column>`
inside the grid.

Our column sizes are specified by the number of columns they'll be spanning. If
we use `<Column lg={4}>`, it means it'll span 4 of the 16 columns. If we use
`<Column lg={8}>` it means it'll span 8 of the 16 columns, and so on. Without
specifying a size, the column will simply span 1 of the 16 columns.
...
return (
  <Grid className="bx--grid--full-width landing-page">
    <Column className="landing-page__banner" lg={16}>
      1
    </Column>
...
the expected `grid` > `col` DOM structure.
...
@use 'carbon-components/scss/globals/scss/vendor/@carbon/type/scss/font-family.scss';
@use 'carbon-components/scss/globals/scss/vendor/@carbon/layout/scss/breakpoint.scss';
@use 'carbon-components/scss/globals/scss/typography.scss';
@use 'carbon-components/scss/globals/scss/vars.scss';
```
...
```scss path=src/content/LandingPage/_landing-page.scss
@use '@carbon/react/styles/spacing' as *; //this allows us to use spacing tokens
...
```scss path=src/content/LandingPage/_landing-page.scss
@use '@carbon/react/styles/spacing' as *;
@use '@carbon/react/styles/type' as *;
@use '@carbon/react/styles/breakpoint' as *;
...
return (
  <Grid className="bx--grid--full-width bx--grid--no-gutter repo-page">
    <Column className="repo-page__r1" lg={16}>
