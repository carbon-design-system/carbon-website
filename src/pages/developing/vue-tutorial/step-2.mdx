...
`<div class="bx--grid">`. We can continue to make rows by adding a
`<div class="bx--row">` inside the grid, as well as make columns within those
...
[spacing tokens](/guidelines/spacing/overview#spacing-scale). In a new file
...
Adding these tokesn, mixins etc. here means we can import them with a single
...
We also need to adust our vertical spacing and type treatment. Like before, it's
a matter of using spacing and type tokens like so:
