...
description:
  ...
  app with the Carbon Design System.
tabs:
...
A
[preview](https://carbon-tutorial-web-components-git-step-2-carbon-design-system.vercel.app/)
of what you will build:

<Preview
  height="200"
  title="Carbon Next Tutorial Step 1"
  src="https://carbon-tutorial-web-components-git-step-2-carbon-design-system.vercel.app/"
  frameborder="no"
...
inport in `main.js` from `import './style.css';` to `import './style.scss';`.
...
Before give the button something to do, add the following to `style.scss` which
will allow our button to theme the app.
...
Then in `main.js` add the following code to hadle the button clicks.
...
Running the application at this point it looks like the button has dissapeared.
...
the `./public` folder) from the `@carbon/icons` pacakge.
...
to display it in a themable way in our application.
...
These styles import the Carbon typeography features, define a layout for our
...
Import th ecomponent in `main.js`
...
Then update the main ttag to include the id `main-content`
...
fix, run `git pull upstream step-1` to merge the changes into your branch, then
...
Finally, visit
[carbon-react-tutorial](https://github.com/carbon-design-system/carbon-tutorial-web-components)
to "Compare & pull request". In doing so, make sure that you are comparing to
`step-1` into `base: step-1`. Take notice of the
...
`rm -rf .pnpm-offline-mirror node_modules && pnpm cache clean && pnpm install`.
