...
Fixed boxes are sized in mini units, as are the fixed dimension of hybrid boxes.

Margin and padding are always applied in fixed mini unit multiples. Boxes are
...
The core concept of the 2x Grid is to divide or multiply by two, forming a
...
padding are applied consistently in both cases.
...
boundaries, fixed sizing scales match column sizes.

#### Hybrid grid

Hybrid grids are also frequent in practice, and hybrid boxes have properties of
both.
...
The margins at the outer edge of the grid are always a fixed size within a
breakpoint, even when columns are fluid. See [breakpoints](#breakpoints) for a
table of margin sizes.

![Margins](images/Layout_overview_Margins.svg)

### Padding

Padding is always a fixed multiple of mini units: 16 pixels at all standard
...
![Always place type against the padding.](images/Layout_overview_Paddings-do.svg)
  ...
  <DoDont type="dont" caption="Never place type on the padding.">
...
warrants more separation.

![Gutters](images/Layout_overview_Gutters.svg)

### Breakpoints

Use this set of standard breakpoints to maintain layout integrity across screen
...
window size. We may know content in advance, or the content may be dynamic.

Carefully consider the user's goal for their screen space, particularly when
...
item, scale boxes and use fixed column counts.
...
At each breakpoint, column count is fixed and column width is a multiple of mini
...
[aspect ratios](#aspect-ratio). A grid emerges when each tile uses a multiple of
a consistent base size. Following this method ensures consistency of tile sizes,
even across products.

### Hybrid boxes

Hybrid boxes have different scaling rules in each dimension, so they do not use
aspect ratios. When a user would resize the browser expecting content to scale
in one dimension but not the other, use hybrid boxes.

Use the [sizing scale](#sizing-scale) to size a fixed dimension. Use
...
lines: vertical and horizontal lines on which multiple objects align.

Horizontal and vertical alignment are equally critical. When present, the eye
can follow content more easily, increasing the perception of visual harmony.

## Grid influencers

Within a design system, layouts often follow a common structure, especially when it comes to navigation. Grid influencers are UI elements that shape how content is positioned within the responsive grid. This section outlines which of these patterns you'll
need, where users expect to see them, and how they impact the layout of content.
...
In a UI layout, users expect to find certain types of content in certain areas.
We call these areas _screen regions_. It is especially important for these zones
to be consistent across devices and adapt across breakpoints.
[Learn more on the Global header pattern page](/patterns/global-header).

![Shell Illustration](images/Layout_overview_Screen-regions-1.svg)

<Caption>1. Header 2. Global sidenav 3. Local sidenav</Caption>

![Dropdown Illustration](images/Layout_overview_Screen-regions-2.svg)

<Caption>4. Dropdown menu 5. Content 6. Footer</Caption>

![Dialog Illustration](images/Layout_overview_Screen-regions-3.svg)
...
This panel style floats above the primary content area and does not affect the
...
We provide a sizing scale for both fixed and fluid sizes. Use it to size content
...
sizing icons, tiling boxes, and applying vertical margin space.

![Fixed base unit](images/Layout_overview_Fixed-base-unit.svg)

Choose a size to use as your fixed base unit. Also see the
...
For fluid grids, the column width is the base unit. Just as a box can span
...
etc. Check out the [breakpoints table](#breakpoints) for pixel sizings.
...
When sizing boxes, constrain dimensions to one of the aspect ratios shown below
whenever possible. Doing so will heighten the perception of unity between
products.

Choose an aspect ratio from the table, then multiply the base unit in each
...
For example, to size a fluid box with a 2:1 landscape aspect ratio, you could
...
it by 4 in width and 2 in height, for a fixed box size of 320 by 160.
...
<DoDontRow>
  <DoDont
    type="do"
    caption="Mini unit is the guide when needed. Spacer snaps to the text box and does not necessarily need to snap to the mini unit.">
...
component is cramped, use the minor sizing scale:

When multiple components adhere to the same scale, grid relationships naturally
