...
You’ve learned the grid fundamentals, now let’s put them togther. A good layout
...
For instance, is it more of a long-form reading experience or is the user
scanning a dashboard or completing a multi-step task? Each type of experience
and purpose will require a different layout.

## Content hierarchy

Hierarchy helps users (viewer, reader, audience) to navigate complex concepts
...
The four column layout is the backbone of most IBM experiences—not just on
IBM.com. Software users also like more breathing room as they enter a product,
along with consistent typographic alignments, as they navigate deeper. As a
result, IBM product UIs rely heavily on the four column structure to ease users
into denser content.

Even though there are examples of two- and three-column containers in the
...
library](https://v1-ibm-products.carbondesignsystem.com/?path=/story/overview-getting-started--page)
...
example for how product designers should mix and match repeatable layout
patterns at both the large and small scales throughout and experience.
...
Unlike editorial experiences, product focuses more on task completion and
...
The Cloud team recently did an audit on type alignment in the
...
_(IBMers only)_ within their products in order to streamline their experiences.
...
<Caption>
  ...
  the dashboard into the product keeps the user oriented.
</Caption>

#### Using Aspect ratios

Aspect ratios also ensure that there is a consistent visual flow as users
...
text with image, pairing UI components and elements of different sizes,
...
![](images/usage/10_Contrast_do.png)
...
![](images/usage/11_Contrast_dont.png)
...
relevant portions of it here too to reach the maximum audience.
...
panels come into play when a user needs to reference the page along with panel
information to complete a task.

Since these slide-in side panels were developed and maintained by Carbon for IBM
Products, please visit their site for
[more in-depth details](https://pages.github.ibm.com/cdai-design/pal/components/side-panel/usage/)
_(IBMers only)_ on side-panel sizes and best practices.
...
models connect specific usage combinations to the kind of content it best
serves.
...
| Product and docs model       | For long form content with deep organizational hierarchy, use the product and docs model. It anticipates a left hand navigational panel, and keeps content within a maximum width. Examples of documentation include most of IBM product UI, IBM Docs, Cloud Docs, and this site documenting the design system guidance.                     |
| High-density interface model | Sometimes every inch of the screen needs be utilized at maximum to display information and controls. Complex product interfaces, catalogs and data visualization dashboards typically fall into this category. This model uses the full width of the browser, so the bigger the screen, the more information the user will see.              |
...
<Caption>
  ...
  dark blue below) can expand, or the grid can stretch.
</Caption>
...
<Caption>
  Max width is maintained and the grid is centered in the browser
</Caption>
...
<Caption>Full width, add columns in increments of 2</Caption>
...
uses the the Product and docs style model.
...
<Caption>The Editorial model in context on IBM.com.</Caption>
...
<Caption>The Product and docs model in context on IBM.com.</Caption>
...
Product and editorial experiences overlap slightly in that they both can use the
...
to maximum the use of screen space. By far the most prominent example of this to
date in product is the Cloud Catalog.
...
Carbon’s Figma templates always have a 32px gutter by default, regardless of
...
however, there are three different gutter structu res: wide (default), narrow
and condensed. These three gutter scenarios enable typographic alignment by
...
<Caption>
  Note: this UI example also include a left hand navigation panel in
  implementation, but we’ve removed it for the simplicity of the example.
</Caption>
...
![](images/usage/34_condensed_do.png)
  ...
  <DoDont type="dont" caption="Do keep all type (inside and outside containers) aligned with the column grid.">
...
platforms — especially since our labeled components (input fields, dropdowns
etc.) require a 32px gutter. Because of scenarios like input fields where the
label text doesn’t align to the input text — both the Narrow gutter mode and the
Condensed gutter mode are most often used in conjunction with the Wide gutter
mode.
...
are especially handy because they also offer the grid influencer templates as
well.
