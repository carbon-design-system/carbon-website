...
Every image that is not decorative must include `alt` text with a meaningful description of the image. If the image is decorative, use an empty `alt` attribute; otherwise the screen reader will read the whole image URL if the `alt` is left out.
