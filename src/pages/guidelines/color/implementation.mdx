...
Not all layer set tokens use `$layer` as its token name. Token sets also include
...
Spec each component variant with its corresponding layer set tokens. For elements
...
for inline theming is applying a contrasting theme to a UI shell and side panels.
This is especially common in light themed products or light modes. For example,
the majority of a page can use the White theme while the UI shell and the left
side panel use an inline Gray 100 theme. This type of inline theme pairing
creates a high contrast moment that adds emphasis and focus to a workflow.
...
theme component assign the new theme to the whole zone.
...
components whether Carbon, PAL or custom made should be redlined using the Carbon
...
display setting, user profile, or account area. At the moment, the placement
