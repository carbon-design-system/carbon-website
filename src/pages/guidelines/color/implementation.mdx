...
[layering model](/guidelines/color/overview#layering-model) within a single
theme, either by using explicit layer tokens or smart contextual layer tokens.
Both ways are acceptable with different pros and cons that should be weighed in
deciding which to use when building your product.
...
There are four sets of color layering within a theme: base layer, layer 01,
layer 02, and layer 03. After each layer of color is added you move up to the
next layering level and use the next set of layering tokens. For example, on top
of `$layer-01` you would place `$layer-02`. These layer set tokens are
identified by a number suffix (-01, -02, -03) attached to the core token name,
such as `$field-03`. The number indicates which set the token is a part of.

Notice, not all layer set tokens use `$layer` as its token name. Token sets also
include `border` and `field` tokens as well as interactive state tokens for the
various layers. Field is considered a layer level and uses the next level token
from the layer it is placed on. Border tokens pair with its same number, so
...
**Coming soon:** Developer documentation is currently be worked on. Check back
...
for inline theming is applying a contrasting theme to UI Shell and side panels.
This is especially common in light themed products or light modes. For example,
the main page content will use the White theme while the UI Shell and the left
side panel uses an inline Gray 100 theme. These high contrast moments created
with inline themes can add emphasis and focus to a work flow.
...
theme component assign the whole zone the new theme.
