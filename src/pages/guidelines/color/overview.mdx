...
<DoDont
    colLg={6}
    caption="Light theme layering model example">
...
![White overflow menu on Gray 10 card.](images/Light_theme_03.png)
  ...
  <DoDont
    colLg={6}
    caption="Dark theme layering model example">

![Gray 80 overflow menu on Gray 90 card.](images/Dark_theme_03_new.png)
...
![Product example in a light theme](images/color-implementation-mode-light.png)
...
![Product example in a light theme](images/color-implementation-mode-dark.png)
...
to help you make correct usage decisions. The token names are divided into 3
possible parts. The first part of the token name references the general UI
element the color is being applied to, like `background`, `text`, `border`. The
second part of token name will specify how or when to use that type of token. It
will be an adjective that describes how or when to use to token, like `subtle`
or `primary`. At its end, token name can also include the interaction state of
the token like `hover` or `active`.
...
`-hover` added after the base token name, such as `$background-hover`.
