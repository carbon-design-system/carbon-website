...
primarily uses the color range of White to Gray 20, and for text and icons uses
...
<Caption>
  Color tokens for components are the same across themes as shown in the White
  theme (left) and Gray 100 theme (right).
</Caption>
...
tempted to use colors from the IBM Design Language directly. For example, when
building a form you want all of the input labels to be Gray 70 (following Carbon
...
Some components have their own explicit color tokens that are only used with
that particular component. These types tokens are called _component tokens_.
They lives with the components and not part of the core color token palette. To
find the components tokens and values for each theme, visit their component
style page.
...
**Coming soon:** Some component tokens may not be available on the component
page yet. Check back later for updates.
...
of the time, this shift in color (to the text or icon element) will be
...
<Caption>
  Shown in the white theme, $layer enabled color (1) and $layer-hover hover
  color (2).
</Caption>
...
<Caption>
  Shown in the White theme, $button-primary enabled color (1) and
  $button-primary-active active color (2).
</Caption>
...
<Caption>
  Shown in the white theme, $layer enabled color (1) and $layer-selected
  selected color (2).
</Caption>
...
`$layer-disabled-01`, while other elements are grouped together and share a
