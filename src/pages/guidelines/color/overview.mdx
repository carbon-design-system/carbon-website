...
the $text-secondary token can dynamically map to Gray 70 or Gray 30 depending on
the theme.
...
like background, text, or border. The second part of token name will specify its
unique role within the element group, like `$border-subtle` or $text-primary.
Additionally, some tokens include an interaction state at the end, like
$background-hover.
