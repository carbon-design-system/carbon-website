...
A consistent approach to capitalization across all that we build contributes greatly to a professional and consistent user experience.
...
- Studies show that this is the easiest and quickest form to read (especially when the text is more than a few words).
...
- Is shown to be more difficult to read (especially when text is more than a few words) as individual letter shapes are less distinguishable from each other when rendered in all caps.
