---
label: Inconsistencies in grammar and style interrupt concentration. Help users focus on your product by following these guidelines and applying them consistently.
title: Content
...
![Capitalization of UI elements](/images/capitalization_of_UI_elements.png)
...
All caps has been shown to be slower to read (especially when text is more than a few words) as individual letter shapes are less distinguishable from each other—they are the same height and have no ascenders or descenders. And it can be difficult for the reader to distinguish between proper nouns and common nouns.

All caps typically requires more space in the UI per letter than sentence case does.
