import HomepageTemplate from '../components/Homepage.js';
import { Tag } from '@carbon/react';

export default HomepageTemplate;

### New to Carbon?

Read through the [introduction to Carbon](/all-about-carbon/what-is-carbon/) first:

<Row className="resource-card-group-3-across">
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="What is Carbon?"
    href="/all-about-carbon/what-is-carbon/"
    actionIcon="arrowRight"
    >
      <MdxIcon name="carbon" />
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="The Carbon ecosystem"
    href="/all-about-carbon/the-carbon-ecosystem/"
    actionIcon="arrowRight"
    >
      <MdxIcon name="carbon" />
  </ResourceCard>
</Column>
</Row>

### Get started

<Row className="image-card-group">
<Column colMd={4} colLg={4} noGutterSm>
    
  <ImageCard
      title="Design"
      titleColor="dark"
      aspectRatio="1:1"
      href="https://carbondesignsystem.com/designing/get-started"
      actionIcon="arrowRight"
      iconColor="dark"
      >

![Design](/homepage/images/homepage-design.png)

  </ImageCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
    <ImageCard
      title="Develop"
      aspectRatio="1:1"
      href="https://carbondesignsystem.com/developing/get-started"
      actionIcon="arrowRight"
      titleColor="dark"
      iconColor="dark"
      >

![Develop](/homepage/images/homepage-develop.png)

  </ImageCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
      <ImageCard
      title="Upgrade"
      aspectRatio="1:1"
      href="https://carbondesignsystem.com/migrating/guide/overview/"
      actionIcon="arrowRight"
      titleColor="dark"
      iconColor="dark"
      >

![Upgrade](/homepage/images/homepage-migration.png)

  </ImageCard>
</Column>
</Row>

### Ecosystem and libraries

Carbon is composed of multiple libraries that build an ecosystem. 
"Core" libraries are maintained by the Carbon team, and community 
libraries extend those core assets to meet specific user needs or 
product requirements. These are maintained by various groups of 
contributors both within and external to IBM.

Looking for a component that you saw in a design or product but can't
find it here? Check community libraries for extended use cases – 
complex components, patterns, guidelines for navigation and more.

In many cases projects using Carbon additionally utilize at least one 
community library.

<Row className="resource-card-group-3-across">
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    title="Carbon for IBM Products"
    href="http://ibm.biz/carbon4ibmproducts"
    >
      <MdxIcon name="carbon" />
      <Tag type="blue">IBMers only</Tag>
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    title="Carbon for Cloud"
    href="https://ibm.biz/carbon4cloud"
    >
      <MdxIcon name="carbon" />
      <Tag type="blue">IBMers only</Tag>
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    title="Carbon for IBM.com"
    href="https://www.ibm.com/standards/carbon/"
    >
      <MdxIcon name="carbon" />
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Community-driven submissions"
    title="Carbon labs"
    href="https://labs.carbondesignsystem.com/"
    >
      <MdxIcon name="github" />
  </ResourceCard>
</Column>
</Row>

The structure of the Carbon documentation follows the following rules.
Each Carbon library has:

<Row>
<Column colMd={5} colLg={8}>
1. **A main website**, like this one for Carbon core, linking the main sources
together and providing a starting point for documentation for both designers
and developers.
1. **Developer-focused documentation** for each library implementation. Usually this is a Storybook site, developers can learn about the code requirements and component APIs. Designers can also use these to research limitations and capabilities of components as they craft designs.
1. A **GitHub repository**, for full visibility of component source code, tests, bug reports, and feature requests.
</Column>
<Column colMd={2} colLg={3} offsetMd={1} offsetLg={1}>
  <Aside>

[Storybook](https://storybook.js.org) is an open source software for documenting and cataloguing front-end components.

  </Aside>
</Column>
</Row>

### Other resources

The component libraries give developers a collection of reusable components for
building websites and user interfaces.

#### Design toolkits

<Row className="resource-card-group-3-across">
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Figma kit"
    href="/designing/kits/figma"
    >
      <MdxIcon name="figma" />
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Sketch kits"
    href="/designing/kits/sketch"
    >
      <MdxIcon name="sketch" />
  </ResourceCard>
</Column>
</Row>

#### Code repositories

<Row className="resource-card-group-3-across">
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon core React components"
    href="https://github.com/carbon-design-system/carbon/tree/main/packages/react"
    >
      <MdxIcon name="github" />
      <Tag type="blue">Official</Tag>
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon core Web Components"
    href="https://github.com/carbon-design-system/carbon/tree/main/packages/web-components"
    >
      <MdxIcon name="github" />
      <Tag type="blue">Official</Tag>
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon core Angular components"
    href="https://github.com/carbon-design-system/carbon-components-angular"
    >
      <MdxIcon name="github" />
      <Tag type="green">Community</Tag>
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon core Vue components"
    href="https://github.com/carbon-design-system/carbon-components-vue"
    >
      <MdxIcon name="github" />
      <Tag type="green">Community</Tag>
  </ResourceCard>
</Column>
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="Carbon core Svelte components"
    href="https://github.com/carbon-design-system/carbon-components-svelte"
    >
      <MdxIcon name="github" />
      <Tag type="green">Community</Tag>
  </ResourceCard>
</Column>
</Row>

### Latest news and articles

<MediumPosts postLimit={3} cardProps={{ color: 'light' }} />
