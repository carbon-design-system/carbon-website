...
<AnchorLink>Migrating a React app</AnchorLink>
<AnchorLink>Migrating an app using just styles</AnchorLink>
<AnchorLink>Migrating an app using Angular, Svelte, Vue, etc.</AnchorLink>
<AnchorLink>Migrating a library</AnchorLink>
...
Now, open your project's root stylesheet and add the following. SHOULD I BE
TELLING THEM TO REMOVE ANYTHING?
...
### Step 4: Update icon sizes and imports
...
1. Removes the `small` boolean prop and replaces it with `size="sm"`.
2. Removes old, v10 size props and replaces them with new, v11 sizes. If you're
   curious about what changes are being made. Check out the _Changes to size_
   section in [step 5](#step-5:-update-components-that-have-changed).
3. Remove the `@carbon/icons-react` pathing and replace it with
   `@carbon/react/icons`.
...
### Step 5: Update components that have changed
...
code. To update, you will need to switch to one of the size options above. The
codemod in [step 4](#step-4:-update-icon-sizes-and-imports) will make short work
of these changes.
...
## Migrating an app using just styles
...
included below, along with what you would need to have at the top of your file
to use it. Essentially, all we are doing is including the path where we would
[find these stylesPOINTS TO A REACT RESOURCE. CONFIRM.](https://github.com/carbon-design-system/carbon/tree/main/packages/react/scss)
in our package.
...
### Step 8: Done with @carbon/styles!
