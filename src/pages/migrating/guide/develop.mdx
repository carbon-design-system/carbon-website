...
One of the biggest changes to Carbon in v11 is that we moved to
...
Now, open your project's main stylesheet:

- Any line of code with `@import` can be removed.
- Any feature flags can be removed.
...
favorite code editor. The prefix`carbon--` is no longer necessary, you can 
do a find for `carbon--` and replace it with nothing to remove across all your files. 
...
Run the following codemod to update your project's icons.
...
Automate size prop changes by running the following codemods.
