...
- Theming: light/dark mode now available along with design token updates
- Design Kits: TBD
- Packages: a new one-package install is available through @carbon/react
- Grid: CSS Grid support has landed to make it easier to build layouts
- React: new one-package install along with new components and API updates
- More consistent API: updating size props to be consistent across all
  components
- Semantic naming: renaming our color tokens to be more intuitive for their use
...
- Some changes with components will require updating to the new component API or
  package.

- For details about what can be done now, see our
