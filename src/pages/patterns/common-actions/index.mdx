...
Use when an action cannot be undone or the item can't be easily recreated. This pattern is also useful if you're deleting more than one thing.
...
For full-page and large modals, keep error messages no longer than 3 paragraph lines. For form errors, keep error messages no longer than two lines.
