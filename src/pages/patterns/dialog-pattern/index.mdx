...
A user action, such as clicking a button, should trigger the dialog to open. Don't interrupt the user by open a dialog when they aren't expecting it. Avoid system generated pop-ups that distract the user while working, like Net Promotor Score requests. Triggers can either be a direct or indirect consequence of a user's action.  An example of an indirect action, is a user closing a tab with unsaved content causing a dialog to ask if they want to save their changes before closing.

If the system is autogenerating an alert that is not the a consequence of a user's action but a response to processes happening in the background then a toast notification should be used instead.
...
Use a modal dialog to confirm user decisions. Clearly describe the action being confirmed and explain any potential consequences that it may cause. Both the title and the button should be reflect the action that will occur. If the action is destructive or irreversible then use a transactional danger modal.
