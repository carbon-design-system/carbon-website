...
### A dialog is a "conversation" between the system and the user which elicits a response or requests information. Dialogs work best when used for short tasks or to alert the user to task relevant information. A dialog is triggered by a user's action, appears on top of the main page content and is persistent until dismissed. 
...
- Use when an immediate response is required from the user.
- Use to request information required to continue the user's current workflow.
- Use to alert the user about pertinent information about the user's current workflow.
- Use to confirm destructive actions.

### When not to use a modal dialog

- Don't use when a response from the user is optional.
- Don't use if the content is not critical or unrelated to the user's current workflow.
- Don't use when another modal is already open
...
| 2   | Body    | Contains the information and/or controls needed to for the modal's task.                             |
| 3   | Footer  | Contains any action needed to complete or cancel the modal task. Content varies based on modal type. |
| 4   | Overlay | Screen overlay that obscures the on page content.                                                    |
...
![scrolling example](images/placeholder.png)
...
There are several possible ways to exit a modal dialog and vary based on type.
...
![dismissal highlights](images/placeholder.png)
...
#### Use dialog modals sparingly

Don't over use modal dialog. Modals are disruptive and can easy annoy the user if used incorrectly or too frequently. When modals are used for non-workflow related task, it is likely a user will start ignoring or dismissing the modals without fully understanding the modals content. This can cause users to make hurried or impulsive choices when dealing with more critical modal dialogs.

#### Modal dialogs should be user initiated

A user action, such as clicking a button, should trigger the dialog to open. Don't interrupt the user by open a dialog when they aren't expecting it. Avoid system generated pop-ups that distract the user while work, like Net Promotor Score requests. Pop-ups like, product promotions or feature showcases should only appear at the beginning or end of a users sessions or workflow and should not interrupt the user while working. This minimizes user annoyance and distractions.

#### Request workflow relevant information from the user

Use a dialog to request information that is preventing the system from continuing a user-initiated process.

#### Notify the user to important information
...
Use a transactional modal dialog to confirm user decisions. Clearly describe the action that the user is trying to take and explain any potential consequences that it may cause. Both the title and the button should be reflect the action that will occur. Use descriptive words like Add, Delete, Save and avoid vague words like Done or OK. If the action is destructive or irreversible then use a transactional danger modal.

#### Keep dialog tasks simple and focused

For tasks that are too small to need a dedicated page and cannot be completed inline, a modal dialog can be used instead. Modal task should be direct and easy to complete without complicated choices and decisions.
Examples include, updating account permissions or editing an apps information.

#### Break longer task up into steps

For longer tasks, use a progress modal to give the user a sense of completion and orientation within the focused flow. However, for complex flows with complex choices, consider using a full page instead of a modal. A progress modal is not a solution for excess modal content. It should only be used to present information in more consumable and focused chunks.

#### Modals prevent access to the main page

While a modal dialog is active a user cannot check content in the main page and is restricted to only the information in the modal for making decisions. Modal tasks should be easy to complete with the limited information presented in the dialog itself. If a user needs access to additional information or reference then consider using a full page instead.
...
### When to use a modeless dialog

- Use for optional or non-critical tasks
- Use to display additional information
- Use when access to the page content is necessary

### When not to use a modeless dialog
...
![modeless types](images/placeholder.png)
...
![modeless structure](images/placeholder.png)
...
| --- | ------- | ------------------------------------------------------------------------------------------- |
| 1   | Header  | Includes a title, optional label and the close icon.                                        |
| 2   | Body    | Contains the information and/or controls needed to for the dialog's task.                   |
| 3   | Footer  | Contains any action needed to preform the dialog task. Content varies based on dialog type. |
...
There are several possible ways to exit a modeless dialog and vary based on dialog type.

For transactional dialogs:
...
When placing buttons, Cancel is also be the furthest left button option and the primary action is always the furtherest right button. There should only ever be one primary actions per dialog. Dialog buttons are always full bleed and attached to the bottom of a dialog.
...
When 3 buttons are needed, each is 25% of the dialog width and aligned to the right side of the dialog. Only the furtherest right button is allowed to be a primary button with the other two being secondary buttons. If all three action have the same weight then all three should be secondary buttons.
...
![button placement examples](images/placeholder.png)
...
On small screens where buttons labels may be too long for the buttons when aligned horizontal, the two buttons can be stack vertically.
