...
![Example of a disclosure opening and closing.](images/disclosure-opening-closing.gif)
...
![Example of a disclosure with multiple sections.](images/disclosure-footer-content.png)
...
| Context menu             | Context menus are used to provide an additional list of options to take on the element it was triggered from.                                                                                                          |
| Setting and filter menus | Settings and filter menus are frequently used within data table toolbars to enable users to adjust content. Filter and sort menus can also be used for filtering large amounts of content on a page or an entire page. |
...
1. **Toolbar profile image:** A profile image, initials, or icon of a user. See
   the user
   [profile image](https://pages.github.ibm.com/cdai-design/pal/patterns/user-profile-images)
   guidance for more information.
...
A context menu is considered to be a disclosure pattern that is triggered by a
right click interaction and opens a popover. A context menu is a defined list of
options in a menu that appears above all other content on a page and is related
...
![Anatomy of a context menu.](images/disclosures-context-menu-anatomy.png)

</Column>
</Row>

<Caption>Anatomy of a context menu.</Caption>

1. **Main menu:** The main and most commonly used menu option.
2. **Submenu (optional):** A secondary menu that contains additional information
   related to a main menu option.
3. **Menu option:** An action that can be performed or an option that can take
   selection.
4. **Divider (optional):** To group related content by creating visual sections.
5. **Checkmark icon:** Indicator that an option in the menu is selected.
6. **Keyboard shortcut icon:** Indicator to provide a quick way of performing an
   action.
7. **Caret icon:** Indicator that there is more information displayed in a
   submenu.

### Structure

A context menu consists of a main menu and can contain additional submenus. Use
dividers to separate content and group into related sections. Use submenus for
main menu options that have additional options to choose from. Options can also
be selectable within the menu depending on the use case.

<Row>
<Column colLg={12}>

![Example of context menu structure.](images/disclosures-context-menu-structure.png)
...
A context menu opens by `right clicking` or by pressing `control + click` on the
content related to the menu on the page. The context menu appears next to the
cursor or where the pointer was placed when clicked. Users can open submenus by
hovering on the submenus option in the main menu. Users can select a menu option
by clicking on it. Clicking on an option will close the menu. Users can also
close the menu by clicking anywhere on the page outside of the menu.

<Row>
<Column colLg={12}>

![Example of context menu hover functionality.](images/disclosures-context-menu-functionality-hover.png)
...
- Upon opening the context menu, the entire main menu receives focus. To give
  focus to the first menu option press `Tab` .
- To move focus to different menu options in the context menu press the `Up` and
  `Down` arrow keys.
- To open a submenu, make sure the main menu option item has focus and press
  `Enter`, `Space`, or the `Right` arrow key.
- To select a menu option press `Enter` or `Space` .
- To close the profile menu press `Enter`, `Space`, or `Esc` .

<Row>
<Column colLg={12}>

![Example of context menu focus functionality.](images/disclosures-context-menu-functionality-focus.png)
...
#### Use in data grid cells

Context menus are typically used in data grid cells to reveal more actions or
options to choose from that are related to the current context of the cell.

#### Use in text editors

Context menus are frequently used in text editors by highlighting text and
activating a menu to show more actions or options to choose from that are
related to the highlighted text.

#### Restrict additional submenus to one level

Context menus can contain multiple different submenus. Submenus can be difficult
to navigate without accidentally dismissing the context menu. Limiting the
amount of submenu levels will help users better navigate the context menu and
reduce complexity.

#### Avoid setting a default option

If options in a context menu are selectable, let the user make the decision to
select an option instead of predefining it for them.
...
- `Tab` to the primary default button to apply focus.
- Trigger the primary default button by pressing `Enter` or `Space`.

Icon button:

- `Tab` to the icon button to apply focus.
- Open the menu by pressing `Enter`, `Space`, or the `Down` arrow key.

Menu:

- When the menu is opened, focus will go to the first additional action in the
  menu list.
- Move between menu actions by pressing the `Up` and `Down` arrow keys.
- Close the menu by pressing `Enter` or  `Space` to activate the action with
...
Context menu,
[Delivering Relevant Tools for Tasks](https://www.nngroup.com/articles/contextual-menus/),
(Nielsen Norman Group)

Split Buttons,
[Component definition](https://www.nngroup.com/articles/split-buttons/),
(Nielsen Norman Group)

Popup,
[Component research](https://open-ui.org/components/popup.research#popup),
(OpenUI)

Enabling Popups,
[Initial Explainer](https://open-ui.org/components/popup.research.explainer),
(OpenUI)

Disclosure,
[W3C WAI-ARIA practices](https://v11.carbondesignsystem.com/components/popover/usage/%5Bhttps://www.w3.org/TR/wai-aria-practices-1.1/#disclosure),
