...
<AnchorLink>Selection filters</AnchorLink>
...
Too many filters can also overcomplicate an experience. Consider how much data the user has to work through and how many attributes they will realistically filter by before designing your filter menu.

### Types of filters

| Type                  | When to use                                                                                                                                                                                                |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _Single selection_    | When the user can pick only one attribute to modify data results. Each filter is applied instantly to the data.                                                                                            |
| _Multi-selection_     | When the user has the option to pick more than one attribute to modify data results. Each selection is applied instantly to the data.                                                                      |
| _Multiple categories_ | When the user has the option to select attributes within multiple data categories. Each filter is applied instantly to the data. If you want to run the filters all at once, use batch filtering.          |
| _Batch filtering_     | When all filters are applied together at the end of the selection process. Can be used for more complex filtering processes and larger data sets to reduce processing load by applying all filters at once |

## Selection filters

### Single selection

Use a single-selection filter when the user can pick **only one attribute** to modify data results. Under the hood, single selection behaves like a [radio button](/components/radio-button/usage). Types of selection filters include:
...
Use a multi-selection filter when the user has the option to pick **more than one attribute** to modify the data results. Under the hood, multi-selections behave like [checkboxes](components/checkbox/usage). Types of single-selection filters include:
...
<Caption>Example of a milti-selection filter in a checkbox set</Caption>
...
Multiple category selection is typically a vertical list on the left side of the page or within a horizontal drawer at the top of the data set. Multiple categories should never be put within a menu or dropdown.
...
Use a batch filter when all filters are applied together at the end of the selection process. The data set only refreshes once at user action. The trigger is most commonly an "Apply filter" button.
...
<Caption>Example of empty hidden filters</Caption>
...
<Caption>Example of hidden filters applied</Caption>
