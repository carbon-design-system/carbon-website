...
<Caption>
  A fluid text input and fluid buttons inside a fluid modal container on the
  condensed grid.
</Caption>
...
default counterpart.The most common types components that have a fluid style are
inputs, buttons, containers, and forms.
...
flow/layout. They must hug at least on edge of a container. Use fixed buttons if
additional options or actions follow below the primary flow and primary action.
...
For more guidance on default versus fluid buttons, see the
...
#### Hybrid fluid buttons

Additionally, there is a hybrid scenario where a floating default-style button
can span a designated number of columns on the responsive column grid, giving it
a fluid width.
...
![Example of a fluid and default form](/images/fluid-pattern-buttons-2.png)
