...
- Limit placeholder text to unfamiliar input types or situations where syntax requirements may be in question (e.g. date formats).
...
Vertical spacing between form sections also depends on whether the form is a dedicated page or a container. Spacing between groups should be adjusted in relationship to spacing between individual items. For instance, if vertical spacing between individual inputs is 24px consider a 32px spacer between sections. If the former number is 32px consider 40px for the latter etc.
