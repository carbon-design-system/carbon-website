...
Forms are incredbily common in user interfaces and their design and usage continues to evolve as input methods get smarter and more and more people use mobile and tablet devices. You might design a form for a user to:
...
<Caption>Dedicated page forms</Caption>
...
First, we should only be asking the user for essential input and we should elicit that information concise, deliberate way. So we should assume that the user scrolls through the appropriate inputs before submitting a form.
...
![Do arrange primary and secondary buttons at the bottom of the form](./images/FPO.png)
...
![Do not top align primary and secondary action buttons in your layouts](./images/FPO.png)
...
![Do use task-specific language in your buttons](./images/TEST_1.png)
...
![Do not use vague language to describe an action](./images/TEST_2.png)
...
We recommend validating the users data before form submission. This type of real-time, inline validation (a.k.a. client-side validation) should happen as soon as the field loses focus. This will help to easily identify the elements that need to be corrected.
...
Inline notifications come into play when server-side errors are involved — i.e. the user tries to submit a form in it’s entirety and the page is reloaded with the detected errors.
...
In-line editing enables users to edit form text in situ instead of taking users to another page to edit their entry—so users don’t have to refresh the the whole form in order to make an edit.
...
Regardless of context, we recommend using a larger spacer between the heading and the first input; all subsequent inputs will have the same margin top unless the inputs are divided into sections.
...
Vertical spacing between sections form sections also depends on wheter the form is a dedicated page or a container. Spacing between groups should be adjusted in relationship to spacing between individual items. For instance, if vertical spacing between individual inputs is 24px consider a 32px spacer between sections… if the former number is 32px consider 40px for the later etc.
...
- See the [dialog pattern](/patterns/dialog-pattern) for more detailed guidance.
...
**Bound entry controls** <br/>
[Date picker](/components/date-picker/usage) <br/>
[Time picker](http://react.carbondesignsystem.com/?path=/story/timepicker--default) <br/>
[Number input](components/number-input/usage) <br/>
[Slider](/components/slider/usage) <br/>
...
[Dialog](/patterns/dialog-pattern)
