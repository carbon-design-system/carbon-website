...
5. **Remember ID (optional):** Saves the user ID and presents the completed input field the next time the user logs in; it is located under the required field. Clarify what is being remembered (that is, “user ID") to avoid confusion.
...
Carbon provides best practice advice on the login pattern but will leave more specific design guidance to the subsystem. For instance, decisions like where to position the login flow on a page (that is, left, right, or center), or whether to use fluid or fixed inputs, can be made at the subsystem level as long as the fields remain on the grid. Designers can also choose whether to incorporate brand-approved subsystem background textures, illustrations and/or marketing content. Visit the [IBM Brand Center](https://www.ibm.com/brand/) for specific guidance and approved assets relating to your subsystem.
