...
### Matching the DOM order to the visual order

- Techniques outlined by the WCAG recommend the visual layout out of your UI objects should match the order they appear in your source code.
- Keyboard users may have trouble predicting where the focus will go if the visual hierarchy does not match the order of the elements in the source code.
- Users who use a combination of screen reader and screen magnifier may get confused when they order doesn't match.
- Add visual that shows DOM next to HTML next to the UI.

#### Meaningful sequence

- The relative order for breadcrumbs, wizards, or Anchor links matters if the nav items reflect the content's meaningful sequence.
- What about provisioning or logging in? Things that take multiple steps and screens.

#### Tab order

What info can we provide about how tab order relates to navigation?

#### Navigation's role as a landmark region

- Specify which region should assume the role of a navigation landmark. This allows accessability tools to treat navigation objects accordingly to match user expectations. [ref](https://www.w3.org/TR/wai-aria-1.1/#dfn-role)
- For assistive technologies like screen readers and rotors.
...
- Web Content Accessibility Guidelines 2.1, [_Understanding Success Criterion 1.3.2: Meaningful Sequence_](https://www.w3.org/WAI/WCAG21/Understanding/meaningful-sequence.html) (2017-2018)
