...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Universal             | The highest level. The hub that connects separate sites which will have their own global navigation.                                 |
| Global                | The level below universal. More focused information. Switch between areas in a site. See navigating [between areas ](#between-areas) |
| Local                 | The lowest level of navigation. This is used to move within a category. See navigating [within an area](#within-an-area)             |
...
<Caption>The header links in the role of global navigation.</Caption>
...
The primary, or local, navigation takes users between areas of an offering. Generally, these ‘areas’ are collections of pages that should be grouped together so a user can undertake an end to end workflow without changing areas.
...
<Caption>
    ...
    3. Lef nav in the accessibility tree <Br/>
</Caption>
</Column>
</Row>
...
Examples of common landmark regions include: navigation, main, form, and search
...
- Ref consistency from WCAG
- Do's and don'ts

## Touch interfaces

Things to consider for devices with touch input. The nav may not always be visible on small screens. The hit area will need to be larger. You lose hover state.

- Loss of hover states. Icons-only buttons miss their label if they only appear on hover. User has to click to understand the category.

- Touch target size. [Apple recommends](https://developer.apple.com/design/human-interface-guidelines/ios/visual-design/adaptivity-and-layout/) 44pt for touch targets and the W3c's [proposed target size](https://www.w3.org/WAI/GL/mobile-a11y-tf/wiki/Proposed_SC_Target_Size) is 48 x 48 css pixels for touch input.
