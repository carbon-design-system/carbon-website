...
**The location** determines the importance and persistence of the navigation elements in your UI. When we talk about global, local, or tertiary in our guidance we are only talking about the location and the level of permanency in the UI.

| Location  | Definition                                                                                                                                                                                                                                                                                                                     |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Global    | The global navigation is present wherever your user goes in your UI. The global navigation contains elements your user may need any time in your UI such as system navigation or globally reoccurring actions. The context shift between the global navigation elements may be drastic as they switch between unrelated tasks. |
| Local     | The local navigation takes users between areas in an offering. Generally, these ‘areas’ are collections of pages that should be grouped together so a user can undertake an end to end workflow without changing areas. A local navigation should only appear in context and only contain product level tasks.                 |
| Tertiary  | The third most important nav to your user’s task. This navigation changes subsets of information on a page.                                                                                                                                                                                                                    |
...
| Task               | Definition                                                                                                                                                                                                                                               |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| System             | System level tasks are not directly tied to the main purpose of your offering or the user's main tasks in your UI. System navigation should let your user navigate to the main sections of your site or access utilities like notifications or settings. |
| Product            | The context shift should not be as drastic since this nav is for related tasks.                                                                                                                                                                          |
| Utility navigation |  A collection of utilities your user expects to be accessed anywhere in your UI. These can be links in the header or footer that aren’t tied to a primary user’s journey.                                                                                |
...
A universal navigation allows for a user to discover and navigate to other sites that may have no other connection other than existing within the same organization's universe of offerings. The need for this highest level of navigation in your UI should be considered carefully and rarely be necessary. A task initiated in a site should note require your user to navigate to another to complete.

The UI Shell's [switcher](https://www.carbondesignsystem.com/components/UI-shell-header/usage#header-elements) is the only component that is appropriate to use as universal navigation. These universal items should remain unchanged in order and when switching between these different sites.

![Diagram and image of the switcher being used as a universal navigation.](images/universal.png)

<Caption>The switcher being used to navigate between universal sites.</Caption>

#### Global

The global navigation lets your user navigate to the main sections of your site. In this example, the [ui shell header](https://www.carbondesignsystem.com/components/UI-shell-header/code) links are used as the global navigation.

![Diagram and image of the header nav being used as a global navigation.](images/global.png)

<Caption>
  Header links and links in the app switcher in the role of global navigation.
</Caption>
...
<Caption>
  The UI shell left panel being used to navigate within an offering.
</Caption>
