...
| Toast              | Short, time-based messages that slide in and out of a page and provide nondisruptive information                  | Assertive toast notifications disappear automatically or can be dismissed by user. Polite toast notifications persist until dismissed by user.   |
...
UI. There are two types of toast notifications.

Assertive toast notifications do not contain action buttons and disappear
automatically after a set amount of time or can also be dismissed by the user.
Assertive notifications take focus when triggered and cannot be tabbed into. Use
these for urgent messages since they disrupt keyboard and screenreader users.

Polite toast notifications can contain action buttons as well as remain on
screen to allow a user to take action or dismiss the alert. These notifications
do not interrupt the screen reader and keyboard workflows so are typically
considered less intrusive than the assertive toast notification.

<Row>
<Column colLg={8}>

![Examples of polite and assertive toast notifications](/images/8_toast_types.png)

<Caption>
  Example of assertive toast notification (left) and polite toast notification
  (right)
</Caption>
