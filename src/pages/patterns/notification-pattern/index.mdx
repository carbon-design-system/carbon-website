...
It's important to consider the user's context when writing notification messages. Use language that is accessible to the user and will be easily understood. Use a tone that is appropriate for the situation and notificaiton.

Because task-generated notifications are sent in response to user action, it isn't necessary to give users extensive background information. Conversely, system-generated notifications are generally not relevant to the user's current task so you should ensure the user has sufficient context to understand the notification.
