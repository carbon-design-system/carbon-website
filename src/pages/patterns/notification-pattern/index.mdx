...
Notifications typically serve as feedback after an action or task is completed.
However, the callout variant is an exception, as it is used to provide guidance
before users begin a task or take an action and, therefore, does not include
success or error statuses.
