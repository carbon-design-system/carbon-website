...
Read-only states are applied to components when the user is allowed to review but not modify the component. It removed all interactive functions from the component.
...
3. **Icon color change:** Keeping embedded icons in the component for context
...
component’s enabled state. In most cases, elements in the enabled state are present in the read-only state.
...
or modified for its read-only state. Changes to color are used to indicate a modification to
interactive elements and a change in their purpose from interactive to informative.
...
The field background color in components blends in with the overall UI or layer
background for read-only states, which helps differentiate it from the enabled
state.
...
information they contain. This contrasts with disabled components, which cannot be reached by a keyboard. However, read-only components are not operable, meaning users
can neither manipulate nor alter their values.
