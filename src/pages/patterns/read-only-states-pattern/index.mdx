...
description:
  ...
  modify. Read-only states limit or completely remove a component’s interactive
  functions.
---

<PageDescription>

Read-only states are applied to components that users can review but not modify.
Read-only states limit or completely remove a component’s interactive functions.
...
<AnchorLinks>
  ...
  <AnchorLink>Designing with read-only states</AnchorLink>
...
Read-only states are applied to components that can be modified in their enabled
...
## Designing with read-only states
...
Don’t make form inputs disabled if they are meant to be read by the user. Unlike
read-only states, disabled states can not be read by screen readers and do not
pass visual contrast.

<DoDontRow>
<DoDont
type="do"
colMd={6} colLg={6}
caption="Do use if the component is not interactable but can be read by the user.">

![Do use if the component is not interactable but can be read by the user.](./images/read-only-do-1.png)
...
be reached by keyboard.) However, read-only components are not operable—users
