...
Search is an essential pattern for navigation or discovery. We live in the age of search, where a searc engine is most people's primary entry point into the internet. User expectations are high for the way search should work, and consistency is critical.
...
Search helps users find what they are looking for across an application or website.

Use this pattern when:

- A user is searching a website or application
- A user is sorting a large set of information, like in a data table
- A user is looking for something specific within a narrow context

### When not to use

Search shouldn’t be the only way to locate information. Especially when offering a focused search, you should include the option to filter instead of search.

## The solution

Every search must be composed of an input field and a button to run the search query.
