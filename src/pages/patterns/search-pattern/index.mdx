...
FPO -- More detail about global with image.

FPO -- More detail about targeted

FPO -- MOre detial about faceted filtering
...
Targeted search allows users to quickly search within a specific category so they can find more relevant content and filter out unwanted results.

Targeted search should be used for scenarios when users understand what they need. As opposed to global searches, which are performed at a high level in order to locate unknown content.
...
Type-ahead suggestions give a user immediate feedback to the user on their search query. Without having to commit to a search, they can get a preview of results, suggestions of additional search terms, or just find exactly what they were looking for without having to visit a separate page first.
...
Users will often search for keywords that don't return any results. The way a search handles this depends on its scope. If a global search fails to return any results, provide suggestions and helpful resources to ensure the user finds what they are looking for.
