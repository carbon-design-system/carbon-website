...
**Note:** The key word search concept included in this pattern is not currently available for production use. The guidance included reflects our current understanding of this topic, and the pattern is currently open for code contributions.
...
Once a file has been chosen, the file will load in the bottom left of the text area. A file can be removed by clicking the close `x` icon.
...
Text toolbars can be used to save or send text files. Text files can also be saved as drafts to be edited again at a later date. Drafts can be saved with an explicit "Save draft" button or it can autosave depending on the use case.
