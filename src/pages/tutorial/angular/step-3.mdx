...
We'll shortcut this using the Angualr CLI, if you'd like more information head over to [Angular Apollo Installation](https://www.apollographql.com/docs/angular/recipes/angular-cli/) for details.
...
- `apollo-angualr` - Apollo integration for Angular
...
Go ahead and start your app with `npm run start`, or, if your app is running, you'll need to restart it to get access to this token.
...
Add the following import at the top of `RepoPage.js`:
...
Next, we will fetch the data in `ngOnInit()`. Add the folowing code right below the model.header:
...
Our last column in the data table will be a comma-separated list of repository and home page links, so let's create a custom column.
...
and at the top of `prepareData()` add:
...
##### src/repositories/repo-table/repo-table.component.spec.ts
