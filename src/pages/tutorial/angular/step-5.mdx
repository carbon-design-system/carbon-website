...
```
or if you have a lite account (the cf cli won't be able to target the default org, if so you need install [ibmcloud](https://cloud.ibm.com/docs/cli?topic=cloud-cli-getting-started#step1-install-idt)):
...
```
and for lite account:

```bash path=package.json
"deploy": "rm -rf ./dist && yarn build && ibmcloud cf push -f manifest.yml"
```
