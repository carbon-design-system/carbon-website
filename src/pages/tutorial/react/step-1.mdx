...
**Note:** It's important that the `TutorialHeader` returns the Carbon `Header` as its containing element, as we'll later be rendering `TutorialHeader` in `App.js` as a preceeding sibling of `Content`, another UI Shell component. Those components need to live one after another for the UI Shell to properly render.
