...
Next we're going to create a React component called `TutorialHeader` to use with
the UI Shell Carbon component. In the `src` directory, create a `components`
directory and inside of that, a `TutorialHeader` directory. Create the following
files inside `src/components/TutorialHeader`:
...
as it's containing element, as we'll later be rendering `TutorialHeader` in
`App.js` as a preceeding sibling of `Content`, another UI Shell component. Those
...
Next thing we need to is update what we're returning in `App.js` . We currently
just have a button in our content. In order to render our pages correctly, we
need to delete the `Button` component within `Content` (and remove the Button
import).
...
`git pull upstream react-step-1` to merge the changes into your branch, then you
