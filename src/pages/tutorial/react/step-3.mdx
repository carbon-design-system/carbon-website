...
Now that we have that data, let's populate the data table. Replace `console.log(data.organization);` with the following that destructures the `organization` object. Once we have the repositories, we can call our `getRowItems()` helper to build the data table rows.
