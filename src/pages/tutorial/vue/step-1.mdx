...
NOTE: We could have installed these seperately but using the CLI to set this up
...
in the component files or a seperate file if needed.
...
If you are using VSCode then you might want to add the following snippet which
...
Shell in our
[carbon-componets-vue](http://vue.carbondesignsystem.com/?path=/story/components-cvuishell-header)
package.
...
`App.vue` as a preceeding sibling of `Content`, another UI Shell component.
Those components need to live one after another for the UI Shell to properly
render.
...
In `src/components/TuturialHeader/TutorialHeader.vue` simply replace the `href`
