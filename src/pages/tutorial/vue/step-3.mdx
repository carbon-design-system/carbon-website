...
Remove 'rows' constant and it's use in the data declaration and add this computed property.
...
Here in order to switch between the stanard rendering of a data cell we've wrapped our standard {{cell}} rendering in a template tag. The template tag is non-rendering so will dissapear leaving us with the same content as before.
